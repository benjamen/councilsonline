...
frappe.provide('lodgeick.timeline');

/**
 * Render timeline visual for Timeframes tab
 */
lodgeick.timeline.renderTimelineVisual = function(frm) {
	if (!frm || !frm.doc) {
 ...
 */
lodgeick.timeline.getMilestoneStatus = function(milestone_date, is_target, is_complete, current_date) {
	if (!milestone_date) {
		return 'pending';
	}
