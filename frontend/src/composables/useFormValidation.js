...
export function useFormValidation(formData, currentStep, isResourceConsent) {
	...
	const hasSubdivision = (formData) => {
		return (
			formData.consent_types?.some((ct) => ct.consent_type === "Subdivision") ||
			false
		)
	}
	...
	const canProceed = () => {
		...
		if (step === 4) {
			...
			const invoiceDetailsValid =
				...
				(!!formData.value.invoice_name?.trim() &&
					!!formData.value.invoice_email?.trim())

			const depositValid =
				!formData.value.transfer_deposit_required ||
		...
		if (step === 6 && isResourceConsent.value) {
			...
			console.log(
				"Step 6 Validation - hasConsentTypes:",
				hasConsentTypes,
			...
			const hasDurations = consentTypes.every((ct) => {
				...
				return (
					durationData.duration_unlimited ||
			...
			const consentNoticeValid =
				!hasSubdivision(formData.value) ||
			...
			const hasDetailedDescription =
				!!formData.value.detailed_description?.trim()
			console.log(
				"Step 6 Validation - hasBriefDescription:",
				hasBriefDescription,
			...
			console.log(
				"Brief:",
				formData.value.brief_description,
				"Detailed:",
		...
		if (step === 8 && isResourceConsent.value) {
			const requiresHazards = formData.value.consent_types?.some(
				(ct) =>
			...
			if (requiresHazards) {
				...
				const hasHazards =
					formData.value.natural_hazards &&
		...
		if (step === 9 && isResourceConsent.value) {
			return (
				!!formData.value.aee_activity_description?.trim() &&
		...
		const isReviewStep =
			(!isResourceConsent.value && step === 6) ||
			(isResourceConsent.value && step === 15)
		if (isReviewStep) {
			return true
		}
