...
describe('Landing', () => {
      ...
      global: {
        stubs: {
          FeatureCard: {
            ...
            template: '<div class="feature-card"><h4>{{ title }}</h4><p>{{ description }}</p></div>'
          },
          ProcessStep: {
            name: 'ProcessStep',
            props: ['number', 'title', 'description'],
            template: '<div class="process-step"><span>{{ number }}</span><h5>{{ title }}</h5><p>{{ description }}</p></div>'
          },
          Button: {
            name: 'Button',
            props: ['size', 'variant', 'theme'],
  ...
  it('renders hero description', () => {
    const wrapper = mountLanding()
    expect(wrapper.text()).toContain('comprehensive platform for ratepayers, civilians, and suppliers')
    expect(wrapper.text()).toContain('Submit consents, service requests, and applications online')
  })

  it('renders Create Account CTA button', () => {
    const wrapper = mountLanding()
    const buttons = wrapper.findAllComponents({ name: 'Button' })
    const createAccountBtn = buttons.find(btn => btn.text().includes('Create Account'))
  ...
  it('renders how it works section heading', () => {
    ...
    expect(wrapper.text()).toContain('Submit your council request in 4 easy steps')
  })
  ...
  it('renders CTA section with heading', () => {
    ...
    expect(wrapper.text()).toContain('Join thousands of ratepayers and suppliers')
  })
  ...
  it('calls goToLogin when Sign In button is clicked', async () => {
    const wrapper = mountLanding()
    const buttons = wrapper.findAllComponents({ name: 'Button' })
    const signInBtn = buttons.find(btn => btn.text().includes('Sign In') && !btn.text().includes('Sign In to Existing'))
  ...
  it('calls goToRegister when CTA Create Free Account button is clicked', async () => {
    ...
    const ctaCreateBtn = ctaButtons.find(btn => btn.text().includes('Create Free Account'))
  ...
  it('calls goToLogin when CTA Sign In button is clicked', async () => {
    const wrapper = mountLanding()
    const ctaButtons = wrapper.findAll('button')
    const ctaSignInBtn = ctaButtons.find(btn => btn.text().includes('Sign In to Existing Account'))
  ...
  it('applies gradient to CTA section', () => {
    const wrapper = mountLanding()
    const ctaSection = wrapper.find('.bg-gradient-to-r.from-blue-600.to-blue-700')
    expect(ctaSection.exists()).toBe(true)
  })
  ...
  it('renders complete landing page structure', () => {
    ...
    expect(wrapper.findAllComponents({ name: 'Button' }).length).toBeGreaterThanOrEqual(3)
  })
})
