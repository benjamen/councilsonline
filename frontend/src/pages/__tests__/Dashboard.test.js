...
vi.mock('../../services', () => ({
  requestService: {
    getUserRequests: () => ({
      get loading() { return mockLoadingState },
      set loading(val) { mockLoadingState = val },
      get data() { return mockRequestsData },
      set data(val) { mockRequestsData = val }
    })
  }
}))
...
vi.mock('../../components/StatCard.vue', () => ({
  default: {
    ...
    template: '<div class="stat-card" :data-title="title" :data-value="value"></div>'
  }
}))
...
describe('Dashboard', () => {
  ...
  it('renders new request button', () => {
    ...
    const newRequestBtn = wrapper.findAll('button').find(btn =>
      btn.text().includes('New Request')
    )
  ...
  describe('Stats Cards', () => {
    ...
    it('displays correct total requests count', () => {
      ...
      const totalCard = wrapper.findAllComponents({ name: 'StatCard' })
    ...
    it('displays correct under review count', () => {
      ...
      const underReviewCard = wrapper.findAllComponents({ name: 'StatCard' })
    ...
    it('displays correct approved count', () => {
      ...
      const approvedCard = wrapper.findAllComponents({ name: 'StatCard' })
    ...
    it('displays correct RFI pending count', () => {
      ...
      const rfiCard = wrapper.findAllComponents({ name: 'StatCard' })
  ...
  describe('Requests Table', () => {
    ...
    it('displays View button for all requests', () => {
      ...
      const viewButtons = wrapper.findAll('tbody button').filter(btn =>
        btn.text() === 'View'
      )
    ...
    it('displays Edit button only for Draft requests', () => {
      ...
      const editButtons = wrapper.findAll('tbody button').filter(btn =>
        btn.text() === 'Edit'
      )
    ...
    it('navigates to request detail when View button clicked', async () => {
      ...
      const viewButton = wrapper.findAll('tbody button').find(btn =>
        btn.text() === 'View'
      )
  ...
  describe('Empty State', () => {
    it('shows empty state when no requests', () => {
      ...
      expect(wrapper.text()).toContain('No applications found')
      expect(wrapper.text()).toContain('Get started by creating your first application')
    })
    ...
    it('shows create button in empty state', () => {
      ...
      const createButton = wrapper.findAll('button').find(btn =>
        btn.text().includes('Create New Application')
      )
