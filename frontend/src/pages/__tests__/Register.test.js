...
describe('Register', () => {
      ...
      global: {
        stubs: {
          ...
          PersonalInfoFields: {
            ...
            props: ['firstName', 'lastName', 'email', 'phone', 'emailError', 'phoneError'],
          ...
          AddPropertyModal: {
            ...
            props: ['show', 'propertyName', 'selectedPropertyAddress', 'isDefault'],
            template: '<div v-if="show" class="add-property-modal"></div>'
          },
          EntityDetailsFields: {
            name: 'EntityDetailsFields',
            props: ['requesterType', 'organizationName', 'companyNumber', 'trustName'],
            template: '<div class="entity-details-fields"></div>'
          },
          PasswordFields: {
            name: 'PasswordFields',
            props: ['password', 'confirmPassword', 'passwordError', 'confirmPasswordError', 'passwordStrength'],
  ...
  it('renders header with correct text', () => {
    ...
    expect(wrapper.text()).toContain('Join thousands using Lodgeick for council requests')
  })
  ...
  it('renders requester option as selected by default', () => {
    ...
    expect(wrapper.text()).toContain('Submit requests for yourself or on behalf of an organization')
  ...
  it('renders agent option with navigation button', () => {
    ...
    expect(wrapper.text()).toContain('Consultants or representatives submitting on behalf of clients')
  ...
  it('renders all form component sections', () => {
    const wrapper = mountRegister()
    expect(wrapper.findComponent({ name: 'AccountTypeSelector' }).exists()).toBe(true)
    expect(wrapper.findComponent({ name: 'PersonalInfoFields' }).exists()).toBe(true)
    expect(wrapper.findComponent({ name: 'PropertySection' }).exists()).toBe(true)
    expect(wrapper.findComponent({ name: 'EntityDetailsFields' }).exists()).toBe(true)
    expect(wrapper.findComponent({ name: 'PasswordFields' }).exists()).toBe(true)
  ...
  it('renders Google SSO button', () => {
    ...
    const googleBtn = buttons.find(btn => btn.text().includes('Sign up with Google'))
  ...
  it('applies card styling to account type selection', () => {
    const wrapper = mountRegister()
    const accountTypeCard = wrapper.findAll('.bg-white.rounded-2xl.shadow-xl')[0]
