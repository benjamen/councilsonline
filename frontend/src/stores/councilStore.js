...
export const useCouncilStore = defineStore("council", {
	...
	getters: {
		...
		selectedCouncilData: (state) => {
			...
			return state.councils.find(
				(c) => c.council_code === state.selectedCouncil,
			)
		},
	...
	actions: {
		...
		async loadCouncilByCode(councilCode) {
			...
			try {
				...
				if (response) {
					...
					const exists = this.councils.find(
		...
		async loadRequestTypesForCouncil(councilCode, forceRefresh = false) {
			...
			if (!forceRefresh && cached && cached.timestamp) {
				...
				if (cacheAge < CACHE_DURATION) {
					console.log(
						"[councilStore] Using cached request types for",
						councilCode,
					)
		...
		setCouncil(councilCode) {
			...
			if (this.isCouncilLocked && councilCode !== this.lockedCouncil) {
				console.warn(
					"Cannot change council - council is locked:",
					this.lockedCouncil,
				)
				return false
			}
			...
			try {
				...
				if (response?.is_locked) {
					...
					if (!this.isCouncilLocked) {
						...
						console.log(
							"[councilStore] Redis has no lock, but preserving in-memory lock:",
							this.lockedCouncil,
						)
					}
				}
