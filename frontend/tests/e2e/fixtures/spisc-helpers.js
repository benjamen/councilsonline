...
export async function findLatestSPISCApplication(page) {
	...
	const listRows = page.locator(
		'.list-row-container, .list-row, [data-doctype="SPISC Application"]',
	)
	...
	if (count > 0) {
		...
		console.log(
			`[SPISC Helper] First row text: ${rowText.substring(0, 100)}...`,
		)
	...
	const clearFilters = page.locator(
		'button:has-text("Clear Filters"), .clear-filters',
	)
	if ((await clearFilters.count()) > 0) {
		...
		if (newCount > 0) {
			...
			if (match) {
				spiscId = match[0]
				console.log(
					`[SPISC Helper] Found SPISC Application after clearing filters: ${spiscId}`,
				)
				return spiscId
			}
		}
	...
	const noData = page.locator(
		'.no-result, .empty-state, :has-text("No SPISC Application found")',
	)
...
export async function getCurrentWorkflowState(page) {
	...
	try {
		const workflowField = page
			.locator(
				'[data-fieldname="workflow_state"] input, [data-fieldname="workflow_state"] .control-value',
			)
			.first()
	...
	if (!state || state === "") {
		try {
			const statusField = page
				.locator(
					'[data-fieldname="status"] input, [data-fieldname="status"] .control-value',
				)
				.first()
	...
	if (!state || state === "") {
		try {
			const indicator = page
				.locator(".indicator-pill, .badge, .status-indicator")
				.first()
...
export async function changeWorkflowState(page, targetState) {
	...
	try {
		...
		if (await actionsButton.isVisible({ timeout: 5000 })) {
			...
			const stateAction = page
				.locator(`.dropdown-menu.show a:has-text("${targetState}")`)
				.first()
...
export async function waitForAssessmentProject(page, timeout = 15000) {
	...
	while (Date.now() - startTime < timeout) {
		...
		const assessmentField = page
			.locator('[data-fieldname="assessment_project"] input')
			.first()
...
export async function navigateToAssessmentProject(page, assessmentId) {
	...
	await page.goto(
		`${BASE_URL}/app/assessment-project/${encodeURIComponent(assessmentId)}`,
	)
	await page.waitForLoadState("networkidle")
	await page.waitForTimeout(2000)
}
...
export async function getAssessmentStageCount(page) {
	...
	const stagesTab = page
		.locator('a:has-text("Stages"), button:has-text("Stages")')
		.first()
	...
	const stageRows = await page
		.locator('[data-fieldname="stages"] .grid-row')
		.count()
...
export async function createTasksFromTemplate(page) {
	...
	try {
		...
		const createTasksButton = page
			.locator('button:has-text("Create Tasks")')
			.first()
...
export async function getTaskCountForRequest(page, requestId) {
	await page.goto(
		`${BASE_URL}/app/project-task?request=${encodeURIComponent(requestId)}`,
	)
	await page.waitForLoadState("networkidle")
	await page.waitForTimeout(2000)
 ...
 * @returns {Promise<boolean>} - Success status
 */
export async function executeSPISCTask(
	page,
	taskId,
	hoursWorked = 1.5,
	...
	try {
		// Navigate to task
		await page.goto(
			`${BASE_URL}/app/project-task/${encodeURIComponent(taskId)}`,
		)
		await page.waitForLoadState("networkidle")
		await page.waitForTimeout(2000)

		// Assign to user if not already assigned
		const assignedToField = page
			.locator('[data-fieldname="assigned_to"] input')
			.first()
		...
		// Record actual time
		const actualTimeField = page
			.locator('[data-fieldname="actual_time"] input')
			.first()
		...
		if (await statusInput.isVisible()) {
			const tagName = await statusInput.evaluate((el) =>
			...
			if (tagName === "select") {
				...
				const completedOption = page
					.locator('.awesomplete li:has-text("Completed")')
					.first()
		...
		// Set progress to 100%
		const progressField = page
			.locator('[data-fieldname="progress"] input')
			.first()
		...
		const saveButton = page
			.locator('button.primary-action:has-text("Save")')
			.first()
...
export async function fillEligibilityAssessment(page, status, notes) {
	...
	try {
		...
		if (await eligibilityField.isVisible()) {
			...
			const tagName = await selectInput.evaluate((el) =>
				el.tagName.toLowerCase(),
			)
 ...
 */
export async function verifyStatusHistoryTransition(
	page,
	fromStatus,
	toStatus,
) {
	...
	const historyTab = page
		.locator('a:has-text("Status History"), button:has-text("History")')
		.first()
	...
	const historyRows = page.locator(
		'[data-fieldname="status_history"] .grid-row',
	)
...
export async function createRFI(
	page,
	requestId,
	questions = ["Please provide additional documents"],
) {
	...
	try {
		...
		// Link to request
		const requestField = page
			.locator('[data-fieldname="request"] input')
			.first()
		...
		for (const question of questions) {
			...
			const addRowButton = page
				.locator('[data-fieldname="questions"] .grid-add-row')
				.first()
			if (await addRowButton.isVisible({ timeout: 2000 })) {
				...
				const questionField = page
					.locator(
						'[data-fieldname="question"] textarea, [data-fieldname="question"] input',
					)
					.last()
