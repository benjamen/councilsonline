...
export async function selectRequestType(page, requestTypeName) {
	...
	console.log(
		`[RequestFlow] Clicked request type containing: ${requestTypeName}`,
	)
}
 ...
 * @param {string} baseUrl - Base URL (default: 'http://localhost:8090')
 */
export async function navigateToNewRequest(
	page,
	councilCode,
 ...
 * @param {string} options.baseUrl - Base URL (default: 'http://localhost:8090')
 */
export async function startSPISCApplication(page, options = {}) {
	const { councilCode = "TAYTAY-PH", baseUrl = "http://localhost:8090" } =
		options
