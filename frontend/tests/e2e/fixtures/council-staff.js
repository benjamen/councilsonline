...
export async function loginAsCouncilStaff(page, options = {}) {
	...
	if (role === STAFF_ROLES.ADMIN || role === 'Administrator') {
		...
		username = email || `test.${role.toLowerCase().replace(' ', '.')}@council.test`;
 ...
 */
export async function changeRequestStatus(page, requestId, newStatus, reason = '') {
	console.log(`[CouncilStaff] Changing request ${requestId} status to: ${newStatus}`);
	...
	const workflowStateField = page.locator('input[data-fieldname="workflow_state"]');
	...
	if (await workflowStateField.count() > 0) {
		...
		if (await workflowButton.count() > 0) {
			...
			console.warn(`[CouncilStaff] Could not find workflow control for status: ${newStatus}`);
			return false;
		}
	}
	...
	if (reason) {
		const reasonField = page.locator('textarea[data-fieldname="reason"], input[placeholder*="reason"]');
...
export async function getCouncilDashboardStats(page) {
	...
	const indicators = await page.locator('.indicator, .stat-card, [class*="stat"]').all();
	...
	for (const indicator of indicators) {
		...
		if (text.includes('RFI')) {
			const match = text.match(/(\d+)/);
			if (match) stats.rfi = parseInt(match[1]);
		}
...
export async function filterRequests(page, filters = {}) {
	...
	// Apply search filter
	if (filters.search) {
		const searchBox = page.locator('.search-input, input[type="search"], .list-search');
...
export async function createTask(page, requestId, taskData = {}) {
	...
	console.log(`[CouncilStaff] Creating task for request ${requestId}: ${subject}`);
	...
	if (await addTaskButton.count() > 0) {
		...
		// Save task
		await page.click('button.btn-primary:has-text("Save"), button.primary-action');
		await page.waitForLoadState('networkidle');
		await page.waitForTimeout(1000);
 ...
 */
export async function waitForAssessmentProject(page, requestId, timeout = 10000) {
	console.log(`[CouncilStaff] Waiting for assessment project for request ${requestId}`);
