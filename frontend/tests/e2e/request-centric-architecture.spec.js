...
test.describe("Request-Centric Architecture E2E Tests", () => {
	test("Complete Request-centric workflow for SPISC", async ({ page }) => {
		...
		const spiscRequestLink = await page
			.locator('a[data-doctype="Request"]')
			.first()
		...
		const viewApplicationBtn = page
			.locator('button:has-text("View Application")')
			.first()
		await expect(viewApplicationBtn).toBeVisible({ timeout: 5000 })
		console.log('✓ "View Application" button exists')

		const viewAssessmentBtn = page
			.locator('button:has-text("View Assessment Project")')
			.first()
		...
		const viewMeetingsBtn = page
			.locator('button:has-text("View Meetings")')
			.first()
		await expect(viewMeetingsBtn).toBeVisible({ timeout: 5000 })
		console.log('✓ "View Meetings" button exists')

		const scheduleMeetingBtn = page
			.locator('button:has-text("Schedule Meeting")')
			.first()
		...
		const viewCommsBtn = page
			.locator('button:has-text("View Communications")')
			.first()
		await expect(viewCommsBtn).toBeVisible({ timeout: 5000 })
		console.log('✓ "View Communications" button exists')

		const sendNotificationBtn = page
		...
		const addNoteBtn = page
			.locator('button:has-text("Add Internal Note")')
			.first()
		...
		const viewDocsBtn = page
			.locator('button:has-text("View Documents")')
			.first()
		await expect(viewDocsBtn).toBeVisible({ timeout: 5000 })
		console.log('✓ "View Documents" button exists')

		const uploadDocBtn = page
			.locator('button:has-text("Upload Document")')
			.first()
		...
		console.log(
			`✓ Dashboard has gradient background: ${bgColor.substring(0, 50)}...`,
		)
		...
		const viewRequestBtn = page
			.locator('button:has-text("View Request (Main Hub)")')
			.first()
		...
		const helperMessage = page.locator(
			"text=Use the Request form to access Tasks",
		)
		...
		const assessEligibilityBtn = page
			.locator('button:has-text("Assess Eligibility")')
			.first()
		if (await assessEligibilityBtn.isVisible()) {
			console.log('✓ SPISC-specific "Assess Eligibility" button exists')
		} else {
			console.log(
				"  (Assess Eligibility button not shown - may be submitted already)",
			)
		}
		...
		const assessmentProjectBtn = page
			.locator('button:has-text("View Assessment Project")')
			.first()
		...
		if (assessmentUrl.includes("assessment-project")) {
			...
			if (await noProjectMsg.isVisible()) {
				console.log(
					"  No Assessment Project exists yet (expected for some requests)",
				)
			}
		}
		...
		const dialogTitle = page.locator(
			'.modal-title:has-text("Send Notification")',
		)
		...
		const assessmentStatusField = page.locator(
			'[data-fieldname="assessment_status"]',
		)
		...
		const assessmentTemplateField = page.locator(
			'[data-fieldname="assessment_template"]',
		)
		...
		const assessmentOwnerField = page.locator(
			'[data-fieldname="assessment_owner"]',
		)
		...
		const assessmentProjectField = page.locator(
			'[data-fieldname="assessment_project"]',
		)
	...
	test("Verify Resource Consent Application integration", async ({ page }) => {
		...
		for (const request of requests) {
			...
			const requestType = await page
				.locator('[data-fieldname="request_type"]')
				.inputValue()
			...
			if (requestType === "Resource Consent") {
				...
				await expect(viewRequestBtn).toBeVisible({ timeout: 5000 })
				console.log(
					'✓ "View Request (Main Hub)" button exists on RC Application',
				)

				// Check for RC-specific buttons (should still exist)
				const refreshConditionsBtn = page
					.locator('button:has-text("Refresh Condition Templates")')
					.first()
				if (await refreshConditionsBtn.isVisible()) {
					console.log(
						'✓ RC-specific "Refresh Condition Templates" button exists',
					)
				}

				// Check for statutory clock indicator
				const clockIndicator = page
					.locator(".statutory-clock-indicator")
					.first()
	...
	test("Verify universal API endpoints work", async ({ page }) => {
		...
		const response = await page.evaluate(async (reqName) => {
			return await frappe.call({
				method: "councilsonline.api.get_request_summary_data",
