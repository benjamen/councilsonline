...
test.describe('Comprehensive Bug Check - Find All Issues', () => {
	...
	test('Complete user journey - find any bugs', async ({ page }) => {
		...
		if (await sendMsgBtn.isVisible().catch(() => false)) {
			...
			const messageModal = page.locator('textarea[placeholder*="message"], textarea[placeholder*="Message"]').first()
			const messageModalVisible = await messageModal.isVisible({ timeout: 2000 }).catch(() => false)
		...
		if (await bookMeetingBtn.isVisible().catch(() => false)) {
			...
			if (modalVisible) {
				...
				const viewSlotsBtn = page.locator('button:has-text("View Available Slots")')
				const viewSlotsVisible = await viewSlotsBtn.isVisible().catch(() => false)
		...
		if (await spiscCard.isVisible({ timeout: 3000 }).catch(() => false)) {
			...
			if (await continueBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
				...
				if (requestId && requestId !== 'new') {
					...
					const sendMsgVisible = await page.locator('button:has-text("Send Message")').isVisible({ timeout: 2000 }).catch(() => false)
					const meetingVisible = await page.locator('h2:has-text("Council Meeting")').isVisible({ timeout: 2000 }).catch(() => false)

					console.log('Send Message visible after auto-save:', sendMsgVisible)
					console.log('Council Meeting visible after auto-save:', meetingVisible)
		...
		// Screenshot
		await page.screenshot({ path: '/tmp/comprehensive-bug-check.png', fullPage: true })
