...
test.describe('Taytay Request - Redirect and Button Visibility', () => {
	test('should show Send Message and Book Meeting buttons on request detail', async ({ page }) => {
		...
		const hasMeetingSection = await meetingHeading.isVisible({ timeout: 5000 }).catch(() => false)

		console.log('Council Meeting section visible:', hasMeetingSection)

		if (!hasMeetingSection) {
			await page.screenshot({ path: '/tmp/request-detail-no-meeting-section.png' })
			console.log('❌ Council Meeting section NOT found - screenshot saved')

			// Check if it's somewhere else
			const bodyText = await page.locator('body').innerText()
			console.log('Page contains "meeting":', bodyText.toLowerCase().includes('meeting'))
			console.log('Page contains "Council Meeting":', bodyText.includes('Council Meeting'))
		}

		expect(hasMeetingSection).toBe(true)
		console.log('✓ Council Meeting section found')

		const bookMeetingButton = page.locator('button:has-text("Request Council Meeting")')
		...
		const hasViewSlots = await viewSlotsButton.isVisible({ timeout: 5000 }).catch(() => false)
	...
	test('should redirect to Taytay dashboard after full request submission', async ({ page }) => {
		...
		// Fill residence
		await page.fill('input[placeholder="123 Main St, Barangay San Juan"]', 'Test Address')
		await page.fill('input[placeholder="Number of years resided in Taytay"]', '10')
		await page.locator('button:has-text("Next")').click()
		await page.waitForTimeout(1000)
		...
		const isGenericDashboard = finalUrl === 'http://localhost:8090/frontend/dashboard' || finalUrl === 'http://localhost:8090/frontend/'
		...
		if (isGenericDashboard) {
			console.log('❌ ISSUE CONFIRMED: Redirecting to generic dashboard instead of Taytay dashboard')
