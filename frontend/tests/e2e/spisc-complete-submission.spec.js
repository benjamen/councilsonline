...
import { ensureLoggedIn, login } from "./fixtures/auth.js"
import {
	clickNext,
...
test.describe("SPISC Complete Submission Flow", () => {
	test("should complete and submit SPISC application successfully", async ({
		page,
	}) => {
		...
		let validationDialog = await page
		...
		if (validationDialog === 0 && submitButtonExists > 0) {
			console.log(
				"Form appears valid and submit button found, attempting submission",
			)
			...
			console.log(
				`  - Validation errors: ${validationDialog > 0 ? "YES" : "NO"}`,
			)
			console.log(
				`  - Submit button found: ${submitButtonExists > 0 ? "YES" : "NO"}`,
			)
			console.log(
		...
		const uploadLogs = consoleLogs.filter(
			(log) =>
		...
		const criticalErrors = errorLogs.filter(
			(log) => log.includes("cannot be a list") || log.includes("417"),
		)
