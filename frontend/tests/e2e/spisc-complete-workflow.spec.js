...
import { fillPersonalInformation, fillHouseholdInformation, fillDeclaration, uploadSPISCDocuments } from './fixtures/spisc-data.js'
...
test.describe('SPISC Complete Workflow E2E', () => {
    ...
    test('Full workflow: Apply → Review → Approve → Payment', async ({ page, browser }) => {
        ...
        await test.step('Phase 1: Applicant fills and submits SPISC application', async () => {
            console.log('\n╔════════════════════════════════════════════════════════════╗')
            console.log('║  PHASE 1: APPLICANT SUBMISSION                             ║')
            console.log('╚════════════════════════════════════════════════════════════╝\n')
            ...
            if (numberMatch) {
                ...
                if (urlMatch) {
                    ...
                    console.log('[Phase 1] ⚠ Could not extract application number from page')
                    // Get the latest application
                    const response = await page.goto(`${BACKEND_URL}/api/resource/SPISC Application?fields=["name"]&limit_page_length=1&order_by=creation desc`)
        ...
        await test.step('Phase 2: Council staff reviews and assesses application', async () => {
            console.log('\n╔════════════════════════════════════════════════════════════╗')
            console.log('║  PHASE 2: COUNCIL STAFF REVIEW                             ║')
            console.log('╚════════════════════════════════════════════════════════════╝\n')
            ...
            await page.goto(`${BACKEND_URL}/app/spisc-application/${encodeURIComponent(applicationNumber)}`)
            await page.waitForTimeout(3000)
            ...
            if (await notesField.isVisible()) {
                await notesField.fill('Applicant meets all SPISC criteria. Verified documentation and eligibility requirements.')
                console.log('[Phase 2] ✓ Added eligibility notes')
            }

            // Save the assessment
            console.log('[Phase 2] Saving assessment...')
            const saveButton = page.locator('button.primary-action, button:has-text("Save")').first()
        ...
        await test.step('Phase 3: Council staff approves application via workflow', async () => {
            console.log('\n╔════════════════════════════════════════════════════════════╗')
            console.log('║  PHASE 3: WORKFLOW APPROVAL                                ║')
            console.log('╚════════════════════════════════════════════════════════════╝\n')
            ...
            if (await requestLink.isVisible()) {
                ...
                await page.goto(`${BACKEND_URL}/app/request/${encodeURIComponent(requestId)}`)
                await page.waitForTimeout(3000)
            }

            // Check current status
            const statusField = page.locator('[data-fieldname="status"]').first()
            const currentStatus = await statusField.inputValue().catch(() => 'Unknown')
            ...
            const actionsButton = page.locator('button:has-text("Actions"), button.btn-default:has-text("Action")').first()
            ...
            if (await actionsButton.isVisible({ timeout: 5000 }).catch(() => false)) {
                ...
                const approveAction = page.locator('text=Approve, a:has-text("Approve")').first()
                if (await approveAction.isVisible({ timeout: 3000 }).catch(() => false)) {
                ...
                console.log('[Phase 3] ⚠ No workflow actions available (may need workflow configuration)')
            }

            // Verify status changed
            await page.waitForTimeout(2000)
            const newStatus = await statusField.inputValue().catch(() => currentStatus)
        ...
        await test.step('Phase 4: Finance staff processes payment', async () => {
            ...
            const paymentMethod = await paymentMethodField.inputValue().catch(() => '')
            ...
            if (paymentMethod) {
                ...
                if (paymentMethod === 'Bank Deposit') {
                    ...
                    if (accountNumber) console.log(`[Phase 4] ✓ Account Number: ${accountNumber}`)
                }
            } else {
                console.log('[Phase 4] ⚠ No payment details found (payment step may need configuration)')
            }
        ...
        await test.step('Phase 5: Applicant views approved application', async () => {
            console.log('\n╔════════════════════════════════════════════════════════════╗')
            console.log('║  PHASE 5: APPLICANT VERIFICATION                           ║')
            console.log('╚════════════════════════════════════════════════════════════╝\n')
            ...
            const isVisible = await applicationCard.isVisible({ timeout: 5000 }).catch(() => false)
            ...
            if (isVisible) {
                ...
                const statusText = await statusBadge.textContent().catch(() => 'Status not found')
        ...
        console.log('╠══════════════════════════════════════════════════════════════╣')
        console.log(`║  Application Number: ${applicationNumber?.padEnd(37) || 'N/A'.padEnd(37)} ║`)
        console.log('║                                                              ║')
        console.log('║  ✅ Phase 1: Application Submitted                           ║')
        console.log('║  ✅ Phase 2: Council Review Complete                         ║')
        console.log('║  ✅ Phase 3: Workflow Approval Processed                     ║')
        console.log('║  ✅ Phase 4: Payment Processing Complete                     ║')
        console.log('║  ✅ Phase 5: Applicant Verification Done                     ║')
        console.log('║                                                              ║')
