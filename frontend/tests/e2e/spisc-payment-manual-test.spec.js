...
test.describe("SPISC Payment Fields Manual Test", () => {
	...
	test("Verify payment step appears and fields work", async ({ page }) => {
		...
		page.on("console", (msg) => {
			const text = msg.text()
			if (
				text.includes("[RequestStore]") ||
				text.includes("[NewRequest]") ||
				text.includes("step")
			) {
		...
		if (pageText.includes("Supporting Documents")) {
			console.log(
				'  ⚠ Found "Supporting Documents" - might be on step 5 instead of step 4',
			)
		}
		if (pageText.includes("Declaration")) {
			console.log(
				'  ⚠ Found "Declaration" - might be on step 6 instead of step 4',
			)
		}
		...
		const headingExists =
			(await page
				.locator("text=Payment Details, text=Payment Method")
				.count()) > 0
		...
		if (!headingExists) {
			console.log(
				"  Payment step is missing - this is expected, continuing test to confirm...",
			)
		...
		const paymentMethodField = page
			.locator("select")
			.filter({ hasText: /Bank Deposit|Cash Pickup/ })
			.or(page.locator('select[name="payment_method"]'))
			.first()
		...
		const accountHolderField = page
			.locator('input[name="account_holder_name"]')
			.first()
		const bankNameField = page.locator('input[name="bank_name"]').first()
		const accountNumberField = page
			.locator('input[name="bank_account_number"]')
			.first()

		const holderVisible = await accountHolderField
			.isVisible()
			.catch(() => false)
		const bankVisible = await bankNameField.isVisible().catch(() => false)
		const numberVisible = await accountNumberField
			.isVisible()
			.catch(() => false)
		...
		if (!holderVisible || !bankVisible || !numberVisible) {
			console.log(
				"  ⚠ WARNING: Bank fields not visible when Bank Deposit selected",
			)
		}
		...
		console.log(
			`  Bank fields hidden: ${holderHidden && bankHidden && numberHidden}`,
		)
		...
		await page.screenshot({
			path: "/tmp/spisc-payment-step.png",
			fullPage: true,
		})
