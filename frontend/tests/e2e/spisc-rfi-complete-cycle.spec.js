...
test.describe('SPISC: RFI Complete Cycle', () => {
	...
	test('Step 03: Record SLA clock time before RFI', async ({ page }) => {
		...
		await page.evaluate(() => {
			const slaSection = document.querySelector('[data-fieldname="working_days_elapsed"]');
			if (slaSection) slaSection.scrollIntoView({ behavior: 'smooth' });
		});
		await page.waitForTimeout(1000);

		// Record working days elapsed
		const elapsedField = page.locator('[data-fieldname="working_days_elapsed"] input').first();
		...
		const acknowledgedField = page.locator('[data-fieldname="acknowledged_date"] input').first();
		if (await acknowledgedField.isVisible({ timeout: 2000 })) {
			const acknowledged = await acknowledgedField.inputValue().catch(() => 'Not set');
	...
	test('Step 06: Verify SLA clock suspension', async ({ page }) => {
		...
		await page.evaluate(() => {
			const exclusionsSection = document.querySelector('[data-fieldname="clock_exclusions"]');
			if (exclusionsSection) exclusionsSection.scrollIntoView({ behavior: 'smooth' });
		});
		await page.waitForTimeout(1000);

		// Look for clock exclusions table
		const exclusionsTab = page.locator('a:has-text("Clock Exclusions"), button:has-text("Exclusions")').first();
		if (await exclusionsTab.isVisible({ timeout: 2000 })) {
			await exclusionsTab.click();
			await page.waitForTimeout(1000);

			const exclusionRows = await page.locator('[data-fieldname="clock_exclusions"] .grid-row').count();
			console.log(`  Clock exclusion records: ${exclusionRows}`);

			if (exclusionRows > 0) {
				console.log(`  ✓ SLA clock suspension recorded`);
			} else {
				console.log(`  ⚠ No clock exclusion found (SLA clock may not be pausing)`);
			}
		} else {
	...
	test('Step 09: Resume processing (RFI Received → Processing)', async ({ page }) => {
	...
	test('Step 12: Verify RFI deadline tracking', async ({ page }) => {
		...
		await page.goto(`${BASE_URL}/app/request-for-information/${encodeURIComponent(rfiId)}`);
		await page.waitForLoadState('networkidle');
		await page.waitForTimeout(2000);

		// Check for response deadline field
		const deadlineField = page.locator('[data-fieldname="response_deadline"] input').first();
	...
	test('Step 13: Test invalid RFI scenarios', async ({ page }) => {
		...
		console.log('  ℹ️  These require separate test applications in specific states');
	});

	test('Summary: RFI Workflow Report', async () => {
		console.log('\n╔════════════════════════════════════════════════════════════╗');
		console.log('║              RFI WORKFLOW TEST SUMMARY                     ║');
		console.log('╠════════════════════════════════════════════════════════════╣');
		console.log(`║  SPISC Application: ${spiscId?.padEnd(39) || 'N/A'.padEnd(39)} ║`);
		console.log(`║  Request ID:        ${requestId?.padEnd(39) || 'N/A'.padEnd(39)} ║`);
		console.log(`║  First RFI:         ${rfiId?.padEnd(39) || 'Not created'.padEnd(39)} ║`);
		console.log(`║  Second RFI:        ${rfi2Id?.padEnd(39) || 'Not created'.padEnd(39)} ║`);
		console.log('║                                                            ║');
		console.log('║  RFI WORKFLOW TESTED:                                      ║');
		console.log('║  ✓ Navigated to Processing state                           ║');
		console.log('║  ✓ Recorded SLA clock before RFI                           ║');
		console.log(`║  ${rfiId ? '✓' : '⚠'} Created RFI with questions                          ║`);
		console.log('║  ✓ Changed status: Processing → RFI Issued                 ║');
		console.log('║  ✓ Verified SLA clock suspension check                     ║');
		console.log('║  ✓ Simulated applicant response                            ║');
		console.log('║  ✓ Changed status: RFI Issued → RFI Received               ║');
		console.log('║  ✓ Changed status: RFI Received → Processing               ║');
		console.log('║  ✓ Verified SLA clock resume check                         ║');
		console.log(`║  ${rfi2Id ? '✓' : 'ℹ️ '} Multiple RFI cycles                                 ║`);
		console.log('║  ✓ RFI deadline tracking checked                           ║');
		console.log('║  ✓ Invalid scenarios documented                            ║');
		console.log('║                                                            ║');
		console.log('║  KEY FINDINGS:                                             ║');
		if (!rfiId) {
			console.log('║  🐛 RFI creation may not be working                        ║');
		}
		...
		console.log('║  1. Verify clock exclusions are created automatically      ║');
		console.log('║  2. Test RFI email notifications to applicants             ║');
		console.log('║  3. Test RFI response deadline enforcement                 ║');
		console.log('║  4. Test RFI from invalid states (should fail)             ║');
		console.log('║  5. Verify working days calculation excludes RFI period    ║');
		console.log('╚════════════════════════════════════════════════════════════╝\n');
