...
test.describe("Dropdown Slot Selection - Final Test", () => {
	test("Verify slots load as dropdowns and can be selected", async ({
		page,
	}) => {
		console.log("\n╔═══════════════════════════════════════════════════╗")
		...
		const refreshBtn = page.locator(
			'button:has-text("Refresh Available Slots"), button:has-text("Load Available Slots")',
		)
		...
		if (selectCount >= 4) {
			...
			const options = await firstTimeSlotSelect
				.locator("option")
				.allTextContents()
			...
			if (datetimeCount > 0) {
				console.log(
					"❌ Still showing manual datetime pickers instead of dropdowns",
				)
			}
		}
		...
		await page.screenshot({
			path: "/tmp/dropdown-slots-final.png",
			fullPage: true,
		})
