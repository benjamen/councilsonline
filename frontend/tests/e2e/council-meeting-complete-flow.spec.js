...
test.describe('Council Meeting Complete Flow', () => {
	...
	test('Meeting banner appears during application flow and modal works', async ({ page }) => {
		...
		const btnVisible = await requestMeetingBtn.isVisible({ timeout: 2000 }).catch(() => false)
		...
		await purposeField.fill('I would like to discuss the requirements for my SPISC application and ensure I have all the necessary documentation.')
	...
	test('Meeting banner on RequestDetail page also works', async ({ page }) => {
		...
		// Verify button
		const requestMeetingBtn = page.locator('button:has-text("Request Council Meeting")')
		const btnVisible = await requestMeetingBtn.isVisible({ timeout: 5000 }).catch(() => false)
		console.log('Request Council Meeting button visible:', btnVisible)
		expect(btnVisible).toBe(true)
