...
test.describe("Pre-Application Meeting Booking", () => {
	...
	test("should show meeting banner on request detail page", async ({
		page,
	}) => {
		...
		const meetingSection = page
			.locator('h3:has-text("Need to discuss your application first?")')
			.first()
		await expect(meetingSection).toBeVisible({ timeout: 5000 })

		// Check if button is visible
		const meetingButton = page
	...
	test("should open meeting modal when button clicked", async ({ page }) => {
		...
		await expect(
			page.locator('label:has-text("Meeting Purpose")'),
		).toBeVisible()
		await expect(
			page.locator('label:has-text("Preferred Meeting Times")'),
		).toBeVisible()
	...
	test("should validate required fields and auto-calculate end time", async ({
		page,
	}) => {
		expect(draftId).toBeTruthy()
		...
		await page.fill(
			...
			"I need to discuss my application requirements",
		)
	...
	test("should submit pre-application meeting request", async ({ page }) => {
		...
		await page.fill(
			'textarea[placeholder*="Brief summary"]',
			"Need to discuss eligibility requirements for SPISC",
		)
		await page.fill(
			'textarea[placeholder*="Key topics"]',
	...
	test("should only show meeting banner after request is created", async ({
		page,
	}) => {
		...
		const meetingBannerOnProcessInfo = page.locator(
			'h4:has-text("Need to discuss your application first?")',
		)
