...
test.describe('SPISC Application Backend Workflow', () => {
	...
	test('Complete SPISC backend workflow', async ({ page }) => {
		...
		if (dashboardVisible) {
			...
			console.log('⚠️  Summary dashboard not visible (may require manual verification)');
		}
		...
		for (const groupName of buttonGroups) {
			...
			const groupButton = page.locator(`.page-head button:has-text("${groupName}")`).first();
		...
		const actionsButton = page.locator('.page-head button:has-text("Actions")').first();
		await actionsButton.click();
		await page.waitForTimeout(500);

		const viewAssessmentButton = page.locator('.dropdown-menu.show a:has-text("View Assessment Project")');
		...
		if (await viewAssessmentButton.isVisible()) {
			...
			if (currentUrl2.includes('assessment-project')) {
				...
				const createButton = page.locator('button:has-text("Create Assessment")');
		...
		if (requestId) {
			await page.goto(`http://localhost:8090/app/project-task?request=${encodeURIComponent(requestId)}`);
			await page.waitForLoadState('networkidle');
			await page.waitForTimeout(2000);
			...
			if (taskCount > 0) {
				for (let i = 0; i < Math.min(3, taskCount); i++) {
					...
					const taskTitle = await row.locator('.level-item.ellipsis').first().textContent();
					console.log(`   Task ${i + 1}: ${taskTitle.trim()}`);
				}
			} else {
				console.log('⚠️  No tasks found - they may need to be created manually or via template');
			}
		}
		...
		await page.goto(`http://localhost:8090/app/spisc-application/${applicationName}`);
		await page.waitForLoadState('networkidle');
		await page.waitForTimeout(2000);
		...
		const viewTasksButton = page.locator('.dropdown-menu.show a:has-text("View Tasks")');
		...
		const meetingsButton = page.locator('.btn-group button:has-text("Meetings")');
		await meetingsButton.click();
		await page.waitForTimeout(500);

		const viewMeetingsButton = page.locator('.dropdown-menu.show a:has-text("View Meetings")');
		...
		const commsButton = page.locator('.btn-group button:has-text("Communications")');
		await commsButton.click();
		await page.waitForTimeout(500);

		const viewCommsButton = page.locator('.dropdown-menu.show a:has-text("View Communications")');
		...
		const assessmentButton = page.locator('.btn-group button:has-text("Assessment")');

		if (await assessmentButton.isVisible()) {
			await assessmentButton.click();
			await page.waitForTimeout(500);

			const quickAssessButton = page.locator('.dropdown-menu.show a:has-text("Quick Assess Eligibility")');
			const detailedAssessButton = page.locator('.dropdown-menu.show a:has-text("Detailed Assessment")');
		...
		const applicantSection = page.locator('[data-fieldname="applicant_details_section"]');
		if (await applicantSection.isVisible()) {
			const bgColor = await applicantSection.evaluate(el =>
				window.getComputedStyle(el.closest('.section-head')?.nextElementSibling || el).backgroundColor
			);
			console.log('✅ Applicant section has custom styling');
		}

		const assessmentSection = page.locator('[data-fieldname="assessment_section"]');
		if (await assessmentSection.isVisible()) {
			const bgColor = await assessmentSection.evaluate(el =>
				window.getComputedStyle(el.closest('.section-head')?.nextElementSibling || el).backgroundColor
			);
			console.log('✅ Assessment section has custom styling');

			// Check for "Council Staff Only" label
			const councilLabel = page.locator('.section-label:has-text("Council Staff Only")');
		...
		console.log('  ✓ 5 action button groups (Actions, Tasks, Meetings, Communications, Assessment)');
	...
	test('Verify SPISC Assessment Template exists', async ({ page }) => {
		...
		const searchBox = page.locator('input.form-control[placeholder*="Search"]').first();
		...
		const templateRow = page.locator('.list-row-container:has-text("Social Pension")').first();
	...
	test('Verify SPISC Task Templates exist', async ({ page }) => {
		...
		for (const taskName of expectedTasks) {
			const taskRow = page.locator(`.list-row-container:has-text("${taskName}")`).first();
