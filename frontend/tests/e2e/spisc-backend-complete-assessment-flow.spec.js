...
test.describe("SPISC: Complete Backend Assessment Flow", () => {
	...
	test("Step 04: Navigate to Request and check workflow state", async ({
		page,
	}) => {
	...
	test("Step 05: Acknowledge application (triggers Assessment Project creation)", async ({
		page,
	}) => {
	...
	test("Step 06: Wait for Assessment Project auto-creation", async ({
		page,
	}) => {
	...
	test("Step 07: Navigate to Assessment Project and verify stages", async ({
		page,
	}) => {
		if (!assessmentId) {
		...
		const templateField = page
			.locator('[data-fieldname="assessment_template"] input')
			.first()
	...
	test("Step 08: Create tasks from template", async ({ page }) => {
		...
		if (success) {
			...
			console.log(
				"  This may require manual task creation or template configuration",
			)
		}
	})
	...
	test("Step 10: Execute Stage 1 tasks (Eligibility Verification)", async ({
		page,
	}) => {
		if (taskIds.length === 0) {
	...
	test("Step 11: Execute Stage 2 tasks (Income & Poverty Assessment)", async ({
		page,
	}) => {
		if (taskIds.length < 3) {
	...
	test("Step 12: Verify cost/time rollup to Assessment Project", async ({
		page,
	}) => {
		if (!assessmentId) {
			console.log("⚠ Skipping - no Assessment Project found")
		...
		// Check actual hours
		const actualHoursField = page
			.locator('[data-fieldname="actual_hours"] input')
			.first()
		if (await actualHoursField.isVisible()) {
			...
			const hours = Number.parseFloat(actualHours)
			if (hours > 0) {
	...
	test("Step 13: Fill eligibility assessment on SPISC Application", async ({
		page,
	}) => {
	...
	test("Step 15: Change workflow state to Pending Decision", async ({
		page,
	}) => {
	...
	test("Step 16: Approve application (manager decision)", async ({ page }) => {
		...
		if (success) {
			...
			console.log(
				"  This may require manager permissions or prerequisite completion",
			)
		}
	})
	...
	test("Step 17: Verify payment processing section", async ({ page }) => {
		...
		if (await paymentStatusField.isVisible()) {
			const paymentStatus = await paymentStatusField
				.locator("input, select")
				.inputValue()
				.catch(() => "Not found")
	...
	test("Step 18: Verify status history recorded all transitions", async ({
		page,
	}) => {
	...
	test("Step 19: Final Summary - Report Results", async () => {
		...
		console.log(
			`║  Assessment ID:     ${assessmentId?.padEnd(39) || "N/A".padEnd(39)} ║`,
		)
		console.log(
			`║  Tasks Executed:    ${taskIds.length.toString().padEnd(39)} ║`,
		)
		console.log(
			"║                                                            ║",
		)
		...
		console.log(
			`║    ${assessmentId ? "✓" : "⚠"} Assessment Project ${assessmentId ? "Created" : "Not Created".padEnd(16)} ║`,
		)
		console.log(
			`║    ${assessmentId ? "✓" : "⚠"} Verify Assessment Stages                           ║`,
		)
		console.log(
			`║    ${taskIds.length > 0 ? "✓" : "⚠"} Create Tasks from Template                       ║`,
		)
		console.log(
			`║    ${taskIds.length > 0 ? "✓" : "⚠"} Execute Stage 1 Tasks                             ║`,
		)
		console.log(
			`║    ${taskIds.length >= 3 ? "✓" : "⚠"} Execute Stage 2 Tasks                             ║`,
		)
		console.log(
			`║    ${assessmentId ? "✓" : "⚠"} Verify Cost/Time Rollup                            ║`,
		)
		console.log(
		...
		if (!assessmentId) {
			console.log(
				"║    🐛 Assessment Project auto-creation may not work        ║",
			)
		}
		if (taskIds.length === 0) {
			console.log(
				"║    🐛 Task creation from template may not work             ║",
			)
		}
		if (taskIds.length > 0 && taskIds.length < 10) {
			console.log(
				"║    🐛 Expected 11-12 tasks, found fewer                    ║",
			)
		}
		if (!assessmentId && taskIds.length === 0) {
			console.log(
				"║    ℹ️  Manual verification needed for assessment flow      ║",
			)
