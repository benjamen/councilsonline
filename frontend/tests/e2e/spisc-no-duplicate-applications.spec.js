...
test.describe('SPISC Application - Duplicate Prevention', () => {
    ...
    test('should create only ONE Request and ONE SPISC Application for entire form flow', async ({ page }) => {
        ...
        console.log(`[Test] Request ID after Step 2: ${requestIdAfterStep2} (unchanged ✓)`);
        ...
        await page.fill('input[name="email"]', `test.senior.${Date.now()}@example.com`);
        ...
        console.log(`[Test] Request ID after Step 3: ${requestIdAfterStep3} (unchanged ✓)`);
        ...
        const municipalitySelect = page.locator('select:has-text("Municipality")').first();
        ...
        const response = await page.request.post('http://localhost:8090/api/method/frappe.client.get_count', {
            data: {
                doctype: 'Request',
        ...
        const spiscResponse = await page.request.post('http://localhost:8090/api/method/frappe.client.get_count', {
            data: {
        ...
        console.log(`[Test] SPISC Application count for request ${requestId}: ${spiscCountData.message}`);
        expect(spiscCountData.message).toBe(1);
    });

    test('should verify formData includes request_id after first save', async ({ page }) => {
