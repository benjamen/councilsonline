...
test.describe('Taytay Request Full Flow', () => {
	...
	test('should create Taytay request and redirect to Taytay dashboard', async ({ page }) => {
		...
		await page.click('button:has-text("I Understand - Continue to Application")')
		await page.waitForTimeout(1000)

		// Fill in applicant information (Step 1)
		await expect(page.locator('text=Applicant Information').first()).toBeVisible({ timeout: 5000 })
		...
		await page.fill('input[placeholder="123 Main St, Barangay San Juan"]', '456 Sample Street, Barangay Test')
		await page.fill('input[placeholder="Number of years resided in Taytay"]', '25')
		...
		// Review and Submit
		await expect(page.locator('h2:has-text("Review Your Application")')).toBeVisible({ timeout: 5000 })
		...
		const isTaytayDashboard = currentUrl.includes('/council/TAYTAY-PH/dashboard')
		const isRequestDetail = currentUrl.includes('/request/')
		const isGenericDashboard = currentUrl === 'http://localhost:8090/frontend/dashboard' || currentUrl === 'http://localhost:8090/frontend/'
	...
	test('should show request in dashboard and verify Send Message and Book Meeting buttons', async ({ page }) => {
		...
		const meetingBanner = page.locator('h3:has-text("Need to discuss your application first?")')
		...
		const bookMeetingButton = page.locator('button:has-text("Request Pre-Application Meeting")')
	...
	test('should open meeting modal and show available slots', async ({ page }) => {
		...
		const bookMeetingButton = page.locator('button:has-text("Request Pre-Application Meeting")').first()
		await expect(bookMeetingButton).toBeVisible({ timeout: 5000 })
		...
		const viewSlotsButton = page.locator('button:has-text("View Available Slots")')
		...
		const availableSlotsHeading = page.locator('h4:has-text("Available Time Slots")')
		...
		if (hasSlots) {
			...
			const firstTimeInput = page.locator('input[type="datetime-local"]').first()
			const timeValue = await firstTimeInput.inputValue()
			expect(timeValue).toBeTruthy()
			console.log('✓ Time slot auto-filled:', timeValue)
		} else {
			console.log('⚠ No available slots (expected if no business hours configured or all slots booked)')
		}
	...
	test('should verify council-specific dashboard after login', async ({ page }) => {
		// Navigate to Taytay council dashboard directly
		await page.goto('http://localhost:8090/frontend/council/TAYTAY-PH/dashboard')
		await page.waitForLoadState('networkidle')
		await page.waitForTimeout(2000)
		...
		const hasCouncilName = await councilName.first().isVisible({ timeout: 5000 }).catch(() => false)
