...
test.describe('SPISC: Edge Cases and Error Handling', () => {
		...
		test('Age exactly 60 (boundary condition)', async ({ page }) => {
			...
			if (await ageField.isVisible()) {
				...
				if (age) {
					...
					if (ageNum === 60) {
						console.log('  ✓ Boundary condition: Exactly 60 (should be eligible)');
					} else if (ageNum < 60) {
		...
		test('Application with missing income information', async ({ page }) => {
			...
			if (await incomeField.isVisible()) {
				...
				if (!income || income.trim() === '') {
					...
					console.log('  ℹ️  System should validate income before eligibility assessment');
				} else {
	...
	test.describe('Edge Case 3: Invalid Workflow Transitions', () => {
		test('Cannot skip from Draft to Approved', async ({ page }) => {
			...
			console.log('  ℹ️  Must go through: Draft → Submitted → Acknowledged → Processing → Pending Decision → Approved');
		});

		test('Cannot move backward from Completed', async ({ page }) => {
			console.log('\n=== Edge Case: Backward Transition from Completed ===');

			console.log('  ℹ️  Terminal states (Completed, Declined) should not allow transitions');
			console.log('  ℹ️  Only exception: RFI workflow allows Processing → RFI Issued → RFI Received → Processing');
		});
		...
		test('Cannot approve without eligibility assessment', async ({ page }) => {
			...
			const eligibility = await eligibilityField.locator('input, select').inputValue().catch(() => '');

			if (!eligibility || eligibility === '') {
				console.log('  ⚠ No eligibility assessment');
				console.log('  ℹ️  System should prevent approval without eligibility status');
			} else {
	...
	test.describe('Edge Case 4: Duplicate Prevention', () => {
		test('Check for duplicate applications (same applicant)', async ({ page }) => {
			...
			if (await nameField.isVisible({ timeout: 2000 })) {
				applicantName = await nameField.textContent().catch(() =>
					nameField.locator('input').inputValue().catch(() => '')
				);
			}

			if (!applicantName) {
				const firstNameField = page.locator('[data-fieldname="first_name"]').first();
				const lastNameField = page.locator('[data-fieldname="last_name"]').first();

				const firstName = await firstNameField.locator('input').inputValue().catch(() => '');
				const lastName = await lastNameField.locator('input').inputValue().catch(() => '');
	...
	test.describe('Edge Case 7: Concurrent Editing', () => {
		test('Two users editing same application', async ({ browser }) => {
			...
			await page2.goto(`${BASE_URL}/app/spisc-application/${encodeURIComponent(spiscId)}`);
			await page2.waitForLoadState('networkidle');
			await page2.waitForTimeout(2000);
	...
	test.describe('Edge Case 10: Special Characters', () => {
		test('Handle special characters in names', async ({ page }) => {
			...
			if (await nameField.isVisible()) {
				const name = await nameField.locator('input').inputValue().catch(() => '');
	...
	test('Summary: Edge Cases Report', async () => {
		console.log('\n╔════════════════════════════════════════════════════════════╗');
		console.log('║          EDGE CASES & ERROR HANDLING REPORT                ║');
		console.log('╠════════════════════════════════════════════════════════════╣');
		console.log(`║  SPISC Application: ${spiscId?.padEnd(39) || 'N/A'.padEnd(39)} ║`);
		console.log('║                                                            ║');
		console.log('║  EDGE CASES TESTED:                                        ║');
		console.log('║  ✓ Age boundary conditions (60 years)                      ║');
		console.log('║  ✓ Missing required fields detection                       ║');
		console.log('║  ✓ Invalid workflow transitions documentation              ║');
		console.log('║  ✓ Duplicate application prevention check                  ║');
		console.log('║  ✓ Browser refresh state preservation                      ║');
		console.log('║  ✓ Permission boundaries documented                        ║');
		console.log('║  ✓ Concurrent editing scenario tested                      ║');
		console.log('║  ✓ Empty/null value handling documented                    ║');
		console.log('║  ✓ Network failure scenarios documented                    ║');
		console.log('║  ✓ Special characters handling checked                     ║');
		console.log('║                                                            ║');
		console.log('║  CRITICAL VALIDATIONS NEEDED:                              ║');
		console.log('║  1. Age >= 60 enforcement on submission                    ║');
		console.log('║  2. Required fields validation before save                 ║');
		console.log('║  3. Workflow state validation (no skipping)                ║');
		console.log('║  4. Duplicate application prevention                       ║');
		console.log('║  5. Permission-based action restrictions                   ║');
		console.log('║  6. Eligibility assessment requirement                     ║');
		console.log('║  7. Graceful error handling and user feedback              ║');
		console.log('║                                                            ║');
		console.log('║  RECOMMENDATIONS:                                          ║');
		console.log('║  1. Implement client-side validation for age               ║');
		console.log('║  2. Add server-side duplicate detection                    ║');
		console.log('║  3. Implement optimistic locking for concurrent edits      ║');
		console.log('║  4. Add comprehensive error messages                       ║');
		console.log('║  5. Test with role-specific users (Planner, Manager)       ║');
		console.log('║  6. Implement network failure recovery                     ║');
		console.log('║  7. Add input sanitization for special characters          ║');
		console.log('║  8. Prevent workflow state skipping                        ║');
		console.log('║  9. Add approval prerequisite validation                   ║');
		console.log('║  10. Implement form state persistence (auto-save)          ║');
		console.log('╚════════════════════════════════════════════════════════════╝\n');
