...
test.describe("SPISC: Complete Assessment Workflow with Template, Roles & Time Tracking", () => {
	...
	test("Step 03 - View Assessment Project linked to request", async () => {
		...
		const assessmentLink = page
			.locator(`.list-row:has-text("${requestId}")`)
			.first()
		...
		if ((await assessmentLink.count()) > 0) {
			...
			console.log(
				"⚠ No assessment project found - will be created on acknowledgment",
			)
		}
	})
	...
	test("Step 04 - Check if Assessment Template was applied", async () => {
		...
		if ((await templateField.count()) > 0) {
			const templateValue = await templateField
				.locator("input")
				.inputValue()
				.catch(() => "None")
		...
		const stagesTab = page.locator(
			'.form-tabs a:has-text("Stages"), button:has-text("Stages")',
		)
	...
	test("Step 05 - Check budgeted hours and cost", async () => {
		...
		const budgetedHours = await page
			.locator('[data-fieldname="budgeted_hours"] input')
			.inputValue()
			.catch(() => "0")
		console.log(`Budgeted Hours: ${budgetedHours}`)

		// Check actual hours
		const actualHours = await page
		...
		const estimatedCost = await page
			.locator('[data-fieldname="estimated_cost"] input')
			.inputValue()
			.catch(() => "$0")
	...
	test("Step 06 - Create tasks from template or manually", async () => {
		...
		await page.evaluate(() => {
			const section =
				document.querySelector('[data-label="Assessment"]') ||
		...
		if (isVisible) {
			...
			console.log(
				"⚠ Assessment project field not visible - skipping (field may be conditional)",
			)
		}

		// Set expected time (hours)
		const expectedTimeField = page.locator(
			'[data-fieldname="expected_time"] input',
		)
		...
		const assignedToField = page.locator(
			'[data-fieldname="assigned_to"] input, [data-fieldname="owner"] input',
		)
	...
	test("Step 07 - Record time spent on task", async () => {
		...
		if (taskMatch) {
			...
			// Record actual time
			const actualTimeField = page.locator(
				'[data-fieldname="actual_time"] input',
			)
			if ((await actualTimeField.count()) > 0) {
				...
				if (isReadonly) {
					console.log(
						"⚠ Actual time is read-only - time may be tracked via Timesheet",
					)
				} else {
			...
			const timesheetTab = page.locator(
				'a:has-text("Time"), button:has-text("Time")',
			)
	...
	test("Step 08 - Check cost calculation on task", async () => {
		...
		const costField = page.locator(
			'[data-fieldname="total_costing_amount"] input, [data-fieldname="total_cost"] input',
		)
		if ((await costField.count()) > 0) {
			const cost = await costField
				.first()
				.inputValue()
				.catch(() => "$0")
		...
		const costingTab = page.locator(
			'a:has-text("Costing"), button:has-text("Costing")',
		)
	...
	test("Step 09 - Mark task as completed", async () => {
		...
		const statusField = page.locator(
			'[data-fieldname="status"] input, select[data-fieldname="status"]',
		)
		if ((await statusField.count()) > 0) {
			...
			if (inputType === "text") {
				...
				const completedOption = page.locator(
					'.frappe-control[data-fieldname="status"] .awesomplete li:has-text("Completed")',
				)
	...
	test("Step 11 - View Request and verify workflow state", async () => {
		...
		const workflowState = await page
		...
		if ((await assessmentField.count()) > 0) {
			const assessmentValue = await assessmentField
				.locator("input")
				.inputValue()
				.catch(() => "None")
