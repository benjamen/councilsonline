...
test("Debug RequestDetail page - Check meeting section after resource loads", async ({
	page,
}) => {
	...
	const isMeetingVisible = await meetingSection
	...
	if (!isMeetingVisible) {
		...
		const meetingDivCount = await page
			.locator('div:has(h2:has-text("Council Meeting"))')
			.count()
	...
	await page.screenshot({
		path: "/tmp/request-detail-debug2.png",
		fullPage: true,
	})
