...
test.describe("Meeting Slot Selection UX", () => {
	test("Slot selection should use dropdown selectors, not manual datetime pickers", async ({
		page,
	}) => {
		console.log("\n╔═══════════════════════════════════════════════════╗")
		...
		const loadBtnVisible = await loadSlotsBtn
		...
		if (loadBtnVisible) {
			...
			console.log(
				'Button text might be different, checking for "Refresh Available Slots"...',
			)
			const refreshBtn = page.locator(
				'button:has-text("Refresh Available Slots")',
			)
		...
		await page.screenshot({
			path: "/tmp/slot-selection-ui.png",
			fullPage: true,
		})
		...
		if (selectCount >= 4) {
			...
			if (options > 1) {
				...
				console.log(
					"   ⚠️  No available slots, but dropdown exists for when slots are available",
				)
			}
