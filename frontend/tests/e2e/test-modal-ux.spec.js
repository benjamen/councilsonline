...
test.describe("Modal UX Improvements", () => {
	...
	test("Send Message modal shows success and auto-closes", async ({ page }) => {
		...
		await page.fill(
			...
			"This is a test message to verify the success UX works correctly.",
		)
		...
		if (successVisible) {
			...
			const modalStillVisible = await modalTitle
	...
	test("Book Meeting modal shows success and auto-closes", async ({ page }) => {
		...
		const loadSlotsBtn = page.locator(
			'button:has-text("Load Available Slots"), button:has-text("Refresh Available Slots")',
		)
		...
		await page.fill(
			...
			"I would like to discuss my SPISC application requirements.",
		)
