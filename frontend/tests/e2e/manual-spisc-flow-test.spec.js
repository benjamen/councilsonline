...
test.describe('SPISC Request: Complete Flow from Lodgement to Approval', () => {
		...
		if (totalRequests > 0) {
			...
			if (match) {
				...
				if (await requestTypeField.count() > 0) {
					const requestType = await requestTypeField.locator('input').inputValue().catch(() => 'Unknown');
				...
				if (await stateField.count() > 0) {
					const currentState = await stateField.locator('input').inputValue().catch(() => 'Unknown');
			...
			console.log('⚠ No requests found - you may need to create one first via the frontend');
		}
	});
	...
	test('Step 03 - View request details and UX components', async () => {
		...
		const statusPill = page.locator('.status-pill, .indicator-pill, [class*="status-badge"]');
		...
		const timeline = page.locator('.workflow-timeline, .timeline, [data-component="timeline"]');
	...
	test('Step 04 - Check available workflow actions', async () => {
		...
		const workflowButtons = await page.locator('button[data-action], .btn-workflow, button:has-text("Submit"), button:has-text("Approve"), button:has-text("Acknowledge")').all();
	...
	test('Step 05 - View linked application (if exists)', async () => {
		...
		const spiscLink = page.locator('a[href*="spisc-application"], [data-fieldname="spisc_application"]');
	...
	test('Step 06 - Check for Assessment Project', async () => {
		...
		const assessmentLink = page.locator('a[href*="assessment-project"], [data-fieldname="assessment_project"]');
		...
		if (await assessmentLink.count() > 0) {
			...
			if (href) {
				...
				// Check for stages
				const stages = await page.locator('[data-fieldname="stages"], .assessment-stage').count();
				if (stages > 0) {
			...
			console.log('  No Assessment Project found - may be created after acknowledgment');
		}
	});
	...
	test('Step 07 - View SLA tracking information', async () => {
		...
		const acknowledgedDate = await page.locator('[data-fieldname="acknowledged_date"] input').inputValue().catch(() => '');
	...
	test('Step 08 - View tasks linked to request', async () => {
		...
		const tasksTab = page.locator('.tab-link:has-text("Task"), button:has-text("Task"), a:has-text("Tasks")');
		...
		if (await tasksTab.count() > 0) {
			...
			const taskCount = await page.locator('.task-row, [data-doctype="Project Task"]').count();
	...
	test('Step 09 - View communications log', async () => {
		...
		if (await commTab.count() > 0) {
			...
			const commCount = await page.locator('.communication-row, [data-doctype="Communication"]').count();
	...
	test('Step 10 - Final Summary', async () => {
		...
		// Get final state
		const finalState = await page.locator('[data-fieldname="workflow_state"] input').inputValue().catch(() => 'Unknown');
		const requestType = await page.locator('[data-fieldname="request_type"] input').inputValue().catch(() => 'Unknown');
		...
		console.log(`2. Or implement API calls to transition states programmatically`);
