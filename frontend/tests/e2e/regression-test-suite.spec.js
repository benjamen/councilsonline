...
test.describe('Regression Tests - Critical Bugs', () => {
	...
	test.describe('Bug: CSRF Token Errors (Dec 2025)', () => {
		test('should have CSRF token available in window object', async ({ page }) => {
			...
			const hasCSRFToken = await page.evaluate(() => {
				return typeof window.csrf_token !== 'undefined' && window.csrf_token !== '{{ csrf_token }}'
			})
		...
		test('should successfully make API calls without CSRF errors', async ({ page }) => {
			test.skip(true, 'Requires running server - CSRF initialization code in place')
			...
			// 	const res = await fetch('/api/method/lodgeick.api.get_active_councils', {
			// 		method: 'POST',
			// 		credentials: 'include',
			// 		headers: {
	...
	test.describe('Bug: Document Upload Breaking Draft Save (417 Error)', () => {
		test('rate limiter should allow guest_only=True parameter', async () => {
			...
			const rateLimitPath = path.join(__dirname, '../../../../../lodgeick/utils/rate_limit.py')
			...
			expect(content).toContain('def rate_limit(calls=10, period=60, guest_only=False):')

			// Check the skip logic for authenticated users
			expect(content).toContain('if guest_only and frappe.session.user != "Guest":')
		...
		test('create_draft_request should use guest_only=True', async () => {
			...
			const apiPath = path.join(__dirname, '../../../../../lodgeick/api.py')
		...
		test('authenticated users should not hit rate limit when uploading documents', async ({ page }) => {
	...
	test.describe('Bug: Socket.io Connection Failures', () => {
		test('should gracefully handle socket.io connection failures', async ({ page }) => {
		...
		test('real-time features should degrade gracefully without socket', async ({ page }) => {
			test.skip(true, 'Requires running server')
	...
	test.describe('Bug: Dynamic Steps Issues with SPISC Application', () => {
		test('conditional logic should be enabled for dynamic steps', async () => {
			...
			const filePath = path.join(__dirname, '../../src/components/DynamicStepRenderer.vue')
			const content = fs.readFileSync(filePath, 'utf-8')

			// Should have active conditional logic
			expect(content).toContain('const visibleSections = computed(() => {')
			expect(content).toContain('return props.stepConfig.sections.filter(section => {')
			expect(content).toContain('isSectionVisible(section, localData.value)')

			// Should NOT have the old commented-out code
			expect(content).not.toContain('const visibleSections = computed(() => step.value?.sections || [])')
		...
		test('file upload should have proper handler in DynamicFieldRenderer', async () => {
			...
			const filePath = path.join(__dirname, '../../src/components/DynamicFieldRenderer.vue')
			const content = fs.readFileSync(filePath, 'utf-8')
			...
			expect(content).toContain('const handleFileUploadToFrappe = async (fileData, resolveCallback) =>')
			expect(content).toContain("fetch('/api/method/upload_file'")

			// Should have watcher to convert file arrays to URLs
			expect(content).toContain('// Watch for file uploads and convert arrays to file URLs')
			expect(content).toContain('if (Array.isArray(value) && value.length > 0 && value[0].file_url)')

			console.log('✓ File upload handler properly configured in DynamicFieldRenderer')
		})
		...
		test('should handle council API failures gracefully', async ({ page }) => {
			...
			if (apiErrors.length > 0) {
				...
				const appCrashed = await page.evaluate(() => {
					return document.querySelector('#app')?.textContent?.includes('runtime error')
				})
...
test.describe('Regression Tests - Performance', () => {
	test('should not have N+1 queries in request list', async () => {
		...
		const requestPyPath = path.join(__dirname, '../../../../../lodgeick/doctype/request/request.py')
	...
	test('should have composite database indexes', async () => {
		...
		const requestJsonPath = path.join(__dirname, '../../../../../lodgeick/doctype/request/request.json')
	...
	test('should use lazy loading for modals', async () => {
		...
		const newRequestPath = path.join(__dirname, '../../src/pages/NewRequest.vue')
