...
test("Complete End-to-End: Council Site Flow", async ({ page }) => {
	...
	const newRequestBtn = page.locator(
		'button:has-text("New Request"), button:has-text("New Application"), a:has-text("New Request"), a:has-text("New Application")',
	)
	const hasButton = await newRequestBtn
	...
	if (hasButton) {
		...
		if (url.includes("/request/new")) {
			...
			if (await spiscCard.isVisible({ timeout: 5000 }).catch(() => false)) {
				...
				const sendMessageButton = page.locator(
		...
		console.log(
			"This is expected if council landing page doesn't have direct request creation",
		)
