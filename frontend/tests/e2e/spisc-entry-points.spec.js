...
test.describe("SPISC Entry Points & Authentication", () => {
	...
	test("Entry Point 1: Via Council Portal Homepage", async ({ page }) => {
		...
		const newRequestButton = page.locator(
			'a[href*="/request/new"], button:has-text("New Request"), a:has-text("Services")',
		)
		await newRequestButton
			.first()
			.click({ timeout: 10000 })
			.catch(() => {
	...
	test("Entry Point 2: Direct URL to New Request", async ({ page }) => {
		...
		if (currentUrl.includes("/login")) {
			console.log(
				"[Entry] ✓ Correctly redirected to login (authentication required)",
			)
	...
	test("Entry Point 3: Direct URL to Specific Request Type", async ({
		page,
	}) => {
		...
		const hasSPISC =
			pageContent.includes("SPISC") || pageContent.includes("Social Pension")
	...
	test("Entry Point 4: Browser Back Button Navigation", async ({ page }) => {
		...
		await page.goto(
			`${BASE_URL}/frontend/request/new?council=${COUNCIL_CODE}`,
		...
		const isLoggedIn =
			!content.includes("Sign in") && !content.includes("Login")
	...
	test("Entry Point 5: Bookmark/Saved Link (Cold Start)", async ({ page }) => {
		...
		if (currentUrl.includes("/login")) {
			...
			if (afterLoginUrl.includes("/request")) {
				console.log(
					"✓ Successfully redirected to original destination after login",
				)
			} else {
				console.log(
					"⚠ Redirected to different page, manual navigation may be needed",
				)
			}
		} else {
	...
	test("Entry Point 6: Logout and Re-login Flow", async ({ page }) => {
		...
		console.log(
			"[Entry] After logout, attempting to access request page:",
			afterLogoutUrl,
		)
	...
	test("Entry Point 7: Multiple Tabs/Windows (Session Sharing)", async ({
		browser,
	}) => {
	...
	test("Entry Point 8: Auth Token Expiry Handling", async ({ page }) => {
		...
		if (afterExpiry.includes("/login")) {
			console.log(
				"✓ Correctly detected expired session and redirected to login",
			)
		} else {
		...
		const response = await page.goto(`${BASE_URL}/api/method/ping`, {
			waitUntil: "networkidle",
		})
	...
	test("Entry Point 10: Invalid/Malformed URLs", async ({ page }) => {
		...
		await page.goto(`${BASE_URL}/frontend/request/new?council=INVALID-CODE`, {
			waitUntil: "networkidle",
		})
		await page.waitForTimeout(2000)
		...
		if (
			content1.includes("error") ||
			content1.includes("invalid") ||
			content1.includes("not found")
		) {
		...
		await page.goto(
			`${BASE_URL}/frontend/request/new?council=${COUNCIL_CODE}&type=INVALID-TYPE`,
			{ waitUntil: "networkidle" },
		)
		await page.waitForTimeout(2000)
		...
		await page.goto(`${BASE_URL}/frontend/request/new?council=&type=`, {
			waitUntil: "networkidle",
		})
		await page.waitForTimeout(2000)
