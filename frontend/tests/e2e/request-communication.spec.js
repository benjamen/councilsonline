...
test.describe('Request Communication Features', () => {
	...
	test.beforeEach(async ({ page }) => {
		...
		await page.locator('button:has-text("Close"), button:has-text("Continue Editing")').first().click({ force: true })
		await page.waitForTimeout(500)
	})

	test('should display Send Message button on request detail page', async ({ page }) => {
	...
	test('should open Send Message modal and display all fields', async ({ page }) => {
		expect(draftId).toBeTruthy()
		...
		const sendMessageButton = page.locator('button:has-text("Send Message")').first()
		await sendMessageButton.click()
		await page.waitForTimeout(500)

		// Verify modal opened with correct title
		await expect(page.locator('h3:has-text("Send Message to Council")')).toBeVisible()
		...
		await expect(page.locator('label:has-text("Communication Type")')).toBeVisible()
		...
		await expect(page.locator('text=Your message will be sent to the council processing team')).toBeVisible()
		await expect(page.locator('text=They typically respond within 2-3 business days')).toBeVisible()

		// Verify action buttons - check for the modal's buttons specifically
		const modalButtons = page.locator('div[data-state="open"]')
		await expect(modalButtons.locator('button:has-text("Cancel")')).toBeVisible()
		await expect(modalButtons.locator('button:has-text("Send Message")')).toBeVisible()
	})

	test('should validate required fields in Send Message modal', async ({ page }) => {
		expect(draftId).toBeTruthy()
		...
		const sendButton = page.locator('div[data-state="open"] button:has-text("Send Message")')
		...
		await page.fill('textarea[placeholder*="Type your message"]', 'Test message content')
	...
	test('should successfully send a message', async ({ page }) => {
		...
		// Fill in the form
		await page.fill('input[placeholder*="Brief description"]', 'Test Message Subject')
		await page.fill('textarea[placeholder*="Type your message"]', 'This is a test message to the council regarding my application.')
		...
		const apiPromise = page.waitForResponse(
			response => response.url().includes('/api/method/lodgeick.api.send_request_message'),
			{ timeout: 15000 }
		)
	...
	test('should close Send Message modal on Cancel', async ({ page }) => {
		...
		// Verify modal closed
		await expect(page.locator('h3:has-text("Send Message to Council")')).not.toBeVisible()
	})

	test('should display Book Meeting button for Resource Consent requests', async ({ page }) => {
		...
		if (!hasRC) {
			console.log('No Resource Consent type available, skipping Book Meeting test')
			test.skip()
			return
		}
		...
		const bookMeetingButton = page.locator('button:has-text("Book Council Meeting")')
		await expect(bookMeetingButton).toBeVisible({ timeout: 10000 })
	})

	test('should open Book Meeting modal and display all fields', async ({ page }) => {
		expect(draftId).toBeTruthy()
		...
		const bookMeetingButton = page.locator('button:has-text("Book Council Meeting")')
		const hasButton = await bookMeetingButton.isVisible({ timeout: 2000 }).catch(() => false)

		if (!hasButton) {
			console.log('Book Meeting button not available for this request type, skipping')
			test.skip()
			return
		}
		...
		// Verify modal opened
		await expect(page.locator('h3:has-text("Request Council Meeting")')).toBeVisible()
		...
		await expect(page.locator('h4:has-text("What happens next?")')).toBeVisible()
		await expect(page.locator('text=Your meeting request will be sent to the council processing team')).toBeVisible()
		await expect(page.locator('text=A council planner will contact you within 2 business days')).toBeVisible()

		// Verify buttons
		await expect(page.locator('button:has-text("Cancel")')).toBeVisible()
		await expect(page.locator('button:has-text("Request Meeting")')).toBeVisible()
	})

	test('should display all new sections on request detail page', async ({ page }) => {
		expect(draftId).toBeTruthy()
		...
		await expect(page.locator('h2:has-text("Application Overview")')).toBeVisible()
		await expect(page.locator('h2:has-text("Property Information")')).toBeVisible()
		await expect(page.locator('h2:has-text("Applicant Information")')).toBeVisible()
		...
		const hasAppDetails = await appDetailsHeading.isVisible({ timeout: 2000 }).catch(() => false)
		if (hasAppDetails) {
		...
		const hasEditButton = await editDraftButton.isVisible({ timeout: 2000 }).catch(() => false)
		if (hasEditButton) {
