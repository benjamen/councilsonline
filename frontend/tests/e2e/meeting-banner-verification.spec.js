...
test.describe("Meeting Banner Verification", () => {
	test("Meeting banner shows on NewRequest page and modal works", async ({
		page,
	}) => {
		console.log(
		...
		await page.goto(
			"http://localhost:8090/frontend/request/new?council=TAYTAY-PH&requestType=SPISC",
		)
		await page.waitForLoadState("networkidle")
		...
		let stepCount = 0
		while (
			!(await meetingBanner.isVisible().catch(() => false)) &&
			stepCount < 5
		) {
			console.log(
				`Step ${stepCount}: Meeting banner not visible yet, clicking Next...`,
			)
		...
		if (bannerVisible) {
			...
			console.log(
				"⚠ Meeting banner not found (might be on wrong step or request type)",
			)
	...
	test("Both fixes verified on RequestDetail page", async ({ page }) => {
		...
		const sectionVisible = await meetingSection
			.isVisible({ timeout: 5000 })
			.catch(() => false)
		...
		const btnVisible = await requestBtn
