...
test.describe("Complete SPISC Application Submission", () => {
	...
	test("Complete SPISC application from start to finish", async ({ page }) => {
		...
		await page.click('button:has-text("New Request"), a:has-text("New Request"), button:has-text("New Application"), a:has-text("New Application")')
		await page.waitForLoadState("networkidle")
		await page.waitForTimeout(1000)
		...
		await page.evaluate(() => {
			...
			textInputs.forEach((input, index) => {
				if (!input.value && input.offsetParent !== null) { // visible and empty
					if (input.placeholder?.toLowerCase().includes('name')) {
						input.value = 'Juan Dela Cruz'
					} else if (input.placeholder?.toLowerCase().includes('phone') || input.placeholder?.toLowerCase().includes('mobile')) {
						input.value = '09171234567'
					} else if (input.placeholder?.toLowerCase().includes('address') || input.placeholder?.toLowerCase().includes('street')) {
			...
			selects.forEach(select => {
				if (!select.value && select.offsetParent !== null && select.options.length > 1) {
		...
		while (stepCount < maxSteps) {
			...
			const reviewHeading = page.locator('h2:has-text("Review"), h3:has-text("Review")')
			const isReviewStep = await reviewHeading.isVisible({ timeout: 1000 }).catch(() => false)
			...
			await page.evaluate(() => {
				...
				document.querySelectorAll('input[type="text"], input:not([type])').forEach((input, index) => {
					if (!input.value && input.offsetParent !== null) {
						input.value = input.placeholder?.toLowerCase().includes('name') ? 'Juan Dela Cruz' :
									  input.placeholder?.toLowerCase().includes('phone') ? '09171234567' :
									  input.placeholder?.toLowerCase().includes('address') ? '123 Main Street' :
									  `Test ${index}`
						input.dispatchEvent(new Event('input', { bubbles: true }))
						input.dispatchEvent(new Event('change', { bubbles: true }))
					}
