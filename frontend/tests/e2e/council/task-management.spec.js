...
test.describe('Task Management - Creation & Assignment', () => {
	...
	test('02 - Create new task manually', async () => {
		...
		// Click New button
		const newButton = page.locator('.primary-action, button:has-text("New"), .btn-primary-dark');
	...
	test('03 - Fill task details', async () => {
		...
		// Fill task subject
		const subjectField = page.locator('[data-fieldname="subject"] input, [data-fieldname="title"] input');
	...
	test('04 - Set task priority', async () => {
		...
		if (await priorityField.count() > 0) {
			...
			if (await priorityInput.count() > 0) {
				...
				const highOption = page.locator('li:has-text("High"), [data-value="High"]');
	...
	test('05 - Set due date', async () => {
		...
		const dueDateField = page.locator('[data-fieldname="exp_end_date"] input, [data-fieldname="due_date"] input');
	...
	test('06 - Link task to request', async () => {
		...
		const requestField = page.locator('[data-fieldname="request"], [data-fieldname="linked_request"]');
...
test.describe('Task Management - Completion & Tracking', () => {
	...
	test('08 - Filter tasks by status', async () => {
		...
		// Apply status filter
		const standardFilter = page.locator('.standard-filter, [data-fieldname="status"]');
	...
	test('09 - View task detail page', async () => {
		...
		if (await firstTask.count() > 0) {
			...
			const subjectField = page.locator('[data-fieldname="subject"], [data-fieldname="title"]');
	...
	test('10 - Mark task as completed (if possible)', async () => {
		...
		const openTask = page.locator('.list-row:has-text("Open"), .list-row:has-text("Pending")').first();
		...
		if (await openTask.count() > 0) {
			...
			if (await statusField.count() > 0) {
				...
				const completedOption = page.locator('li:has-text("Completed"), [data-value="Completed"]');
	...
	test('12 - View task cost calculation (if exists)', async () => {
		...
		if (await costField.count() > 0) {
			...
			const costValue = await costField.locator('input').first().inputValue().catch(() => '');
...
test.describe('Task Management - Task Templates', () => {
	...
	test('14 - View task template detail', async () => {
		...
		if (await firstTemplate.count() > 0) {
			...
			const templateNameField = page.locator('[data-fieldname="template_name"], [data-fieldname="name"]');

			if (await templateNameField.count() > 0) {
				const templateName = await templateNameField.locator('input').first().inputValue().catch(() => '');
	...
	test('15 - View template checklist items (if exists)', async () => {
		...
		if (await firstTemplate.count() > 0) {
			...
			if (await checklistTable.count() > 0) {
				...
				const checklistRows = await checklistTable.locator('.grid-row, tr').count();
...
test.describe('Task Management - Timeliness Tracking', () => {
	...
	test('16 - Check for overdue tasks indicator', async () => {
		...
		if (await overdueIndicator.count() > 0) {
			...
			console.log('No overdue indicators found - no overdue tasks or not displayed');
		}
	});
	...
	test('18 - View task dashboard/stats (if exists)', async () => {
		...
		const statCards = page.locator('.indicator-pill, .stat-card, [class*="indicator-"]');
