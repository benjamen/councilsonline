...
test.describe('Request Management - List View & Statistics', () => {
	...
	test('03 - Verify list row count matches stats', async () => {
		...
		const listCount = page.locator('.list-count, .result-count, .list-count-text');
		if (await listCount.count() > 0) {
			const countText = await listCount.textContent();
			const match = countText.match(/(\d+)/);
			if (match) {
				const reportedCount = parseInt(match[1]);
				console.log(`Visible rows: ${visibleRows}, Reported count: ${reportedCount}`);
...
test.describe('Request Management - Search Functionality', () => {
	...
	test('04 - Search by request number', async () => {
		...
		const requestNumber = await firstRow.locator('.level-item:has-text("SPISC"), .level-item:has-text("RC"), .level-item:has-text("BC")').first().textContent();
...
test.describe('Request Management - Filtering', () => {
	...
	test('10 - Combined filters (Status + Type)', async () => {
		...
		console.log(`Combined filter (Draft + SPISC) returned ${resultCount} results`);
...
test.describe('Request Management - Sorting & Pagination', () => {
	...
	test('11 - Sort by creation date (newest first)', async () => {
		...
		const sortButton = page.locator('.sort-selector, button:has-text("Sort"), .sort-by');
	...
	test('12 - Navigate to next page if pagination exists', async () => {
		...
		const nextPageButton = page.locator('.next-page, button:has-text("Next"), .pagination .btn-next');

		if (await nextPageButton.count() > 0 && await nextPageButton.isVisible()) {
	...
	test('13 - Change page size if supported', async () => {
		...
		const pageSizeSelector = page.locator('.page-size, select[name="page_length"], .page-length');
...
test.describe('Request Management - Navigation', () => {
	...
	test('16 - Breadcrumb navigation works', async () => {
		...
		// Look for breadcrumb
		const breadcrumb = page.locator('.breadcrumb, .breadcrumb-link, [class*="breadcrumb"]');

		if (await breadcrumb.count() > 0) {
			// Find "Request" link in breadcrumb
			const requestBreadcrumb = page.locator('.breadcrumb a:has-text("Request")').first();
