...
test.describe('Workflow Lifecycle - Happy Path', () => {
	...
	test('Step 1: Applicant submits Resource Consent application', async () => {
		...
		if (await draftRequest.count() > 0) {
			...
			console.log('No draft request found - test may need request creation setup');
			test.skip();
		}
	...
	test('Step 2: Council staff acknowledges application', async () => {
		...
		const statusField = page.locator('input[data-fieldname="workflow_state"]');
		let currentStatus = await statusField.inputValue();
	...
	test('Step 5: Send to manager (Processing → Pending Decision)', async () => {
		...
		const success = await changeRequestStatus(page, requestId, 'Pending Decision');
		expect(success).toBe(true);
	...
	test('Step 6: Manager approves application', async () => {
		...
		const success = await changeRequestStatus(page, requestId, 'Approved', 'Application meets all requirements');
		expect(success).toBe(true);
	...
	test('Step 7: Verify status history logged all transitions', async () => {
		...
		const timelineSection = page.locator('.form-timeline, .comment-timeline, [data-fieldname="status_history"]');
		...
		if (await timelineSection.count() > 0) {
			...
			const hasAcknowledged = timelineText.includes('Acknowledged') || timelineText.includes('acknowledged');
			const hasProcessing = timelineText.includes('Processing') || timelineText.includes('processing');
			const hasPendingDecision = timelineText.includes('Pending Decision') || timelineText.includes('pending');
			const hasApproved = timelineText.includes('Approved') || timelineText.includes('approved');
			...
			expect(hasAcknowledged || hasProcessing || hasPendingDecision || hasApproved).toBe(true);
		} else {
			console.log('Timeline section not found - status history may be in different location');
		}
	});
	...
	test('Step 8: Verify status badge renders correctly', async () => {
		...
		const statusBadge = page.locator('.badge, .status-pill, [class*="status-"]').first();
...
test.describe('Workflow Lifecycle - All Status Transitions', () => {
	...
	test('Verify all 21 workflow states are reachable', async () => {
		...
		for (const row of listRows.slice(0, 10)) { // Check first 10 requests
		...
		console.log(`Found ${foundStates.size} unique states in request list:`, Array.from(foundStates));
	...
	test('Verify conditional transitions enforce prerequisites', async () => {
		...
		if (await processingRequest.count() > 0) {
			...
			const actionButtons = await page.locator('button[data-action], .workflow-button').all();
