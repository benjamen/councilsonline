...
test.describe("Request Detail View - Basic Information", () => {
	...
	test("02 - Request number matches URL", async () => {
		const requestNumberField = page.locator(
			'[data-fieldname="request_number"] input, [data-fieldname="request_number"]',
		)

		if ((await requestNumberField.count()) > 0) {
			const displayedNumber = await requestNumberField
				.first()
				.inputValue()
				.catch(() => requestNumberField.first().textContent())
	...
	test("03 - Request type is displayed", async () => {
		const requestTypeField = page.locator(
			'[data-fieldname="request_type"] input, [data-fieldname="request_type"]',
		)
		...
		if ((await requestTypeField.count()) > 0) {
			const requestType = await requestTypeField
				.first()
				.inputValue()
				.catch(() => requestTypeField.first().textContent())
	...
	test("04 - Workflow state is displayed", async () => {
		const workflowStateField = page.locator(
			'[data-fieldname="workflow_state"] input',
		)
...
test.describe("Request Detail View - UX Components", () => {
	...
	test("05 - Status badge/pill renders correctly", async () => {
		...
		const statusBadge = page
			.locator('.badge, .status-pill, .indicator, [class*="status-"]')
			.first()

		if ((await statusBadge.count()) > 0) {
			const badgeText = await statusBadge.textContent()
			const badgeColor = await statusBadge.evaluate(
	...
	test("06 - Workflow progression timeline displays", async () => {
		...
		if ((await timeline.count()) > 0) {
			...
			console.log(
				"Workflow progression timeline not found - may be in collapsed section",
			)
		}
	})
	...
	test("08 - Dashboard metrics display (if present)", async () => {
		...
		const dashboard = page.locator(
			'.form-dashboard, [class*="dashboard"], [data-fieldname="status_card_html"]',
		)
...
test.describe("Request Detail View - Linked Records", () => {
	...
	test("10 - Assessment project link (if exists)", async () => {
		...
		if ((await assessmentField.count()) > 0) {
			...
			const assessmentLink = page.locator(
				'[data-fieldname="assessment_project"] a',
			)
			...
			if ((await assessmentLink.count()) > 0) {
				...
				console.log(
					"Assessment project field exists but no assessment linked yet",
				)
			}
		} else {
	...
	test("11 - Linked application navigation (if exists)", async () => {
		...
		const applicationDoctypeField = page.locator(
			'[data-fieldname="application_doctype"]',
		)
		const applicationNameField = page.locator(
			'[data-fieldname="application_name"]',
		)

		if (
			(await applicationDoctypeField.count()) > 0 &&
			(await applicationNameField.count()) > 0
		) {
			const appDoctype = await applicationDoctypeField
				.inputValue()
				.catch(() => "")
	...
	test("12 - Related requests section (if exists)", async () => {
		...
		const relatedRequestsField = page.locator(
			'[data-fieldname="related_requests"]',
		)
...
test.describe("Request Detail View - Actions", () => {
	...
	test("13 - Primary action buttons visible", async () => {
		...
		const menuButton = page.locator(
			".menu-btn, button.btn-secondary, .actions-btn-group button",
		)
	...
	test("14 - Workflow action buttons (if present)", async () => {
		...
		const workflowButtons = page.locator(
			".form-workflow, .workflow-buttons, button[data-action]",
		)
	...
	test("15 - Additional action menu items", async () => {
		...
		const menuButton = page
			.locator('.menu-btn-group .btn, button:has-text("Menu")')
			.first()
...
test.describe("Request Detail View - Edit Functionality", () => {
	...
	test("16 - Update processing notes field", async () => {
		...
		const processingNotesField = page.locator(
			'[data-fieldname="processing_notes"] textarea, [data-fieldname="processing_notes"]',
		)

		if ((await processingNotesField.count()) > 0) {
			const currentValue = await processingNotesField
				.inputValue()
				.catch(() => "")
	...
	test("17 - Update priority field (if exists)", async () => {
		...
		if ((await priorityField.count()) > 0) {
			...
			const priorityInput = page.locator(
				'[data-fieldname="priority"] input, [data-fieldname="priority"] select',
			)
	...
	test("18 - Verify form is dirty after editing", async () => {
		...
		const processingNotesField = page.locator(
			'[data-fieldname="processing_notes"] textarea',
		)
		...
		if ((await processingNotesField.count()) > 0) {
			...
			const dirtyIndicator = page.locator(
				'.indicator-orange, [title*="Not Saved"], .modified-indicator',
			)
