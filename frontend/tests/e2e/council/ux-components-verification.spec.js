...
test.describe("Request Form UX Components Verification", () => {
	...
	test("01 - Load Request detail page without console errors", async () => {
		...
		expect(
			consoleErrors,
			"Request form should load without console errors",
		).toHaveLength(0)
	})
	...
	test("02 - Verify status pill renders correctly", async () => {
		...
		const statusBadge = await page
			.locator('.badge, .status-pill, [class*="status"], [class*="workflow"]')
			.first()
		...
		expect(
			consoleErrors,
			"Status pill should render without errors",
		).toHaveLength(0)
	})
	...
	test("03 - Verify workflow progression timeline loads", async () => {
		...
		const timeline = await page.locator(
			'[class*="timeline"], [class*="progression"], [class*="workflow"]',
		)
		...
		expect(
			consoleErrors,
			"Workflow timeline should load without errors",
		).toHaveLength(0)
	})
	...
	test("04 - Verify dashboard metrics display correctly", async () => {
		...
		const dashboard = await page.locator(
			'.form-dashboard, [class*="dashboard"], .indicator',
		)
		...
		// No errors should be logged
		expect(consoleErrors, "Dashboard should load without errors").toHaveLength(
			0,
		)
	})
	...
	test("05 - Verify requester card renders", async () => {
		...
		const requesterFields = await page.locator(
			'[data-fieldname*="requester"], [data-fieldname*="applicant"]',
		)

		// Just verify no errors during render
		expect(
			consoleErrors,
			"Requester card should render without errors",
		).toHaveLength(0)
	})
	...
	test("06 - Verify no ES6 module errors in action_bar_utils.js", async () => {
		...
		const actionBarErrors = consoleErrors.filter(
			(err) =>
				err.includes("action_bar_utils") ||
				(err.includes("councilsonline") && err.includes("actionBar")),
		)
		expect(
			actionBarErrors,
			"action_bar_utils.js should load without errors",
		).toHaveLength(0)
	})
	...
	test("07 - Verify no ES6 module errors in summary_dashboard.js", async () => {
		...
		const dashboardErrors = consoleErrors.filter(
			(err) =>
				...
				(err.includes("councilsonline") && err.includes("dashboard")),
		)
		expect(
			dashboardErrors,
			"summary_dashboard.js should load without errors",
		).toHaveLength(0)
	})
	...
	test("08 - Verify defensive checks prevent crashes (frm.page.wrapper)", async () => {
		...
		const pageWrapperErrors = consoleErrors.filter(
			(err) =>
				err.includes("frm.page.wrapper") || err.includes("frm.page.title_area"),
		)
		expect(
			pageWrapperErrors,
			"Defensive checks should prevent frm.page errors",
		).toHaveLength(0)
	})
	...
	test("09 - Verify defensive checks prevent crashes (frm.dashboard.wrapper)", async () => {
		...
		const dashboardWrapperErrors = consoleErrors.filter(
			(err) =>
				err.includes("frm.dashboard.wrapper") || err.includes("dashboard.find"),
		)
		expect(
			dashboardWrapperErrors,
			"Defensive checks should prevent frm.dashboard errors",
		).toHaveLength(0)
	})
...
test.describe("Request Form - Multiple Workflow States", () => {
	...
	test("11 - Load multiple requests to test different workflow states", async () => {
		...
		expect(
			consoleErrors,
			"Multiple requests should load without console errors",
		).toHaveLength(0)
	})
})
