...
test.describe("Workflow Edge Cases - Decline Request", () => {
	...
	test("Setup: Find request in Pending Decision state", async () => {
		...
		const pendingRequest = page
			.locator('.list-row:has-text("Pending Decision")')
			.first()
	...
	test("Verify decline reason is logged", async () => {
		...
		const timeline = page.locator(
			'.form-timeline, .comment-timeline, [data-fieldname="comments"]',
		)

		if ((await timeline.count()) > 0) {
			const timelineText = await timeline.textContent()

			const hasDeclineReason =
	...
	test("Verify declined request cannot be approved (final state)", async () => {
		...
		console.log(
			`Available actions from Declined state: ${workflowButtons.length}`,
		)
...
test.describe("Workflow Edge Cases - Cancel Request", () => {
	...
	test("Setup: Find request in early stage", async () => {
		...
		const earlyRequest = page
			.locator('.list-row:has-text("Draft"), .list-row:has-text("Submitted")')
			.first()
...
test.describe("Workflow Edge Cases - Invalid Transitions", () => {
	...
	test("Verify cannot skip from Acknowledged to Approved (must go through Processing)", async () => {
		...
		const acknowledgedRequest = page
			.locator('.list-row:has-text("Acknowledged")')
			.first()
		...
		if ((await acknowledgedRequest.count()) > 0) {
			...
			const approveButton = page.locator(
				'button:has-text("Approve"), button[data-action="Approved"]',
			)

			if ((await approveButton.count()) > 0) {
				console.log(
					"WARNING: Approve button available from Acknowledged - invalid transition may be possible",
				)
			} else {
				console.log(
					"Approve button correctly not available from Acknowledged state",
				)
			}
	...
	test("Verify cannot skip from Draft to Processing", async () => {
		...
		if ((await draftRequest.count()) > 0) {
			...
			const processingButton = page.locator(
				'button:has-text("Start Processing"), button[data-action="Processing"]',
			)

			if ((await processingButton.count()) > 0) {
				console.log(
					"WARNING: Processing button available from Draft - invalid transition may be possible",
				)
			} else {
				console.log(
					"Processing button correctly not available from Draft state",
				)
			}
		} else {
...
test.describe("Workflow Edge Cases - Permission Checks", () => {
	...
	test("Verify Planner cannot approve (only Admin/Manager can)", async () => {
		...
		console.log(
			"Only Council Manager and Administrator roles can approve requests",
		)
	...
	test('Verify "Send to Manager" requires assessment complete', async () => {
		...
		if ((await processingRequest.count()) > 0) {
			...
			if ((await assessmentLink.count()) > 0) {
				...
				const assessmentStatus = page.locator(
					'input[data-fieldname="overall_status"]',
				)
			...
			console.log(
				"Conditional transition check: Send to Manager requires assessment complete",
			)
		} else {
			console.log(
	...
	test("Verify RFI cannot be issued from terminal states", async () => {
		...
		if ((await terminalRequest.count()) > 0) {
			...
			const rfiButton = page.locator(
				'button:has-text("Issue RFI"), button[data-action="RFI Issued"]',
			)

			if ((await rfiButton.count()) > 0) {
				console.log(
					"WARNING: RFI button available from terminal state - may be invalid",
				)
			} else {
...
test.describe("Workflow Edge Cases - Status Rollback", () => {
	...
	test("Verify cannot move backward from Approved to Processing", async () => {
		...
		const approvedRequest = page
			.locator('.list-row:has-text("Approved")')
			.first()
		...
		if ((await approvedRequest.count()) > 0) {
			...
			if ((await processingButton.count()) > 0) {
				console.log(
					"WARNING: Processing button available from Approved - backward transition may be possible",
				)
			} else {
				console.log(
					"Processing button correctly not available from Approved (no backward transitions)",
				)
			}
		} else {
	...
	test("Verify RFI is the only allowed backward transition", async () => {
		...
		const rfiReceivedRequest = page
			.locator('.list-row:has-text("RFI Received")')
			.first()
		...
		if ((await rfiReceivedRequest.count()) > 0) {
			...
			const processingButton = page.locator(
				'button:has-text("Continue Processing"), button[data-action="Processing"]',
			)

			console.log(
				"RFI → Processing is the only valid backward transition allowed",
			)
		} else {
