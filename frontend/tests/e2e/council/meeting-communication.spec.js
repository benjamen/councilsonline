...
test.describe("Meeting Management - Pre-Application Meetings", () => {
	...
	test("03 - Fill meeting details", async () => {
		...
		await page.goto(
			`${BASE_URL}/app/pre-application-meeting/new-pre-application-meeting-1`,
		)
		await page.waitForLoadState("networkidle")
		await page.waitForTimeout(1000)
		...
		const subjectField = page.locator(
			'[data-fieldname="title"], [data-fieldname="subject"]',
		)
		...
		const dateField = page.locator(
			'[data-fieldname="meeting_date"], [data-fieldname="scheduled_date"]',
		)
	...
	test("04 - Set meeting location", async () => {
		...
		const locationField = page.locator(
			'[data-fieldname="location"], [data-fieldname="meeting_location"]',
		)

		if ((await locationField.count()) > 0) {
			await locationField
				.locator("input, textarea")
				.fill("Council Chambers, Level 2")
			await page.waitForTimeout(300)
	...
	test("05 - Add meeting attendees (if field exists)", async () => {
		...
		const attendeesField = page.locator(
			'[data-fieldname="attendees"], [data-fieldname="participants"]',
		)
		...
		if ((await attendeesField.count()) > 0) {
			...
			const addRowButton = page.locator(
				'.grid-add-row, button:has-text("Add Row")',
			)
	...
	test("07 - Record meeting notes/outcome", async () => {
		...
		if ((await firstMeeting.count()) > 0) {
			...
			if ((await notesField.count()) > 0) {
				const noteText =
					"E2E Test: Discussion about site requirements and planning constraints."
...
test.describe("Communication Management - Logging", () => {
	...
	test("10 - Log email communication", async () => {
		...
		if ((await newButton.count()) > 0) {
			...
			if ((await subjectField.count()) > 0) {
				await subjectField
					.locator("input")
					.fill("RE: Application RC-2025-001 - Additional Information")
			...
			if ((await typeField.count()) > 0) {
				...
				const emailOption = page.locator(
					'li:has-text("Email"), [data-value="Email"]',
				)
	...
	test("11 - Log phone call communication", async () => {
		...
		if ((await newButton.count()) > 0) {
			...
			if ((await typeField.count()) > 0) {
				...
				const phoneOption = page.locator(
					'li:has-text("Phone"), li:has-text("Call"), [data-value="Phone"]',
				)
			...
			const contentField = page.locator(
				'[data-fieldname="content"], [data-fieldname="notes"]',
			)

			if ((await contentField.count()) > 0) {
				await contentField
					.locator("textarea")
					.fill("Incoming call from applicant regarding site visit timing.")
	...
	test("12 - View communication timeline on Request", async () => {
		...
		const timeline = page.locator(
			'.form-timeline, .comment-timeline, [class*="timeline"]',
		)
		...
		if ((await timeline.count()) > 0) {
			...
			const timelineEntries = await timeline
				.locator(".timeline-item, .comment-content")
				.count()
	...
	test("13 - Filter communications by type", async () => {
		...
		const standardFilters = page.locator(
			".standard-filter-section, .filter-area",
		)
	...
	test("15 - View email delivery status (if tracked)", async () => {
		...
		if ((await emailComm.count()) > 0) {
			...
			if ((await statusField.count()) > 0) {
				...
				const status = await statusField
					.locator("input")
					.inputValue()
					.catch(() => "")
	...
	test("16 - Attach documents to communication", async () => {
		...
		if ((await firstComm.count()) > 0) {
			...
			const attachSection = page.locator(
				'.attachments-section, [data-fieldname="attachments"]',
			)
			...
			if ((await attachSection.count()) > 0) {
				...
				const attachButton = page.locator(
					'button:has-text("Attach"), .attach-btn',
				)
