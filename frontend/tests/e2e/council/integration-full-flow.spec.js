...
test.describe('Integration Test: Complete RC Application Processing', () => {
	...
	test('Step 02 - Acknowledge application', async () => {
		...
		if (await statusField.count() > 0) {
			const currentStatus = await statusField.locator('input').inputValue().catch(() => '');
	...
	test('Step 03 - Verify Assessment Project auto-creation', async () => {
		...
		const assessmentFound = await waitForAssessmentProject(page, requestId, 15000);
		...
		if (assessmentFound) {
			...
			if (await assessmentLink.count() > 0) {
				...
				const assessmentTitle = await page.locator('.page-title, h1').textContent();
	...
	test('Step 04 - Create tasks from template', async () => {
		...
		if (taskId) {
			...
			console.log('Task creation requires Add Task button - documenting expected flow');
		}
	});
	...
	test('Step 05 - Issue RFI for missing documents', async () => {
		...
		const rfiButton = page.locator('button:has-text("Issue RFI"), button:has-text("Request Information")');
		...
		if (await rfiButton.count() > 0) {
			...
			const messageField = page.locator('textarea[data-fieldname="rfi_message"], textarea[placeholder*="message"]');

			if (await messageField.count() > 0) {
				await messageField.fill('Please provide the following missing documents:\n1. Site plan\n2. Geotechnical report\n3. Environmental impact assessment');

				// Submit RFI
				const submitButton = page.locator('button.btn-primary:has-text("Submit"), button:has-text("Send")');
			...
			console.log('RFI workflow not available from this status - documenting expected behavior');
		}
	});
	...
	test('Step 06 - Verify RFI communication logged', async () => {
		...
		const commTab = page.locator('.tab-link:has-text("Communication"), button:has-text("Communication")');
		...
		if (await commTab.count() > 0) {
			...
			const commRows = await page.locator('.communication-row, .list-row').count();
	...
	test('Step 07 - Schedule site visit meeting', async () => {
		...
		const meetingButton = page.locator('button:has-text("Schedule Meeting"), button:has-text("Add Meeting")');
		...
		if (await meetingButton.count() > 0) {
			...
			const titleField = page.locator('input[data-fieldname="meeting_title"], input[data-fieldname="subject"]');
			...
			console.log('Meeting scheduling not available - documenting expected behavior');
		}
	});
	...
	test('Step 09 - Move to Pending Decision', async () => {
		...
		if (await decisionButton.count() > 0) {
			...
			const confirmButton = page.locator('button.btn-primary:has-text("Confirm"), button:has-text("Yes")');
			...
			console.log('Decision transition not available - may require prerequisite completion');
		}
	});
	...
	test('Step 10 - Manager approves application', async () => {
		...
		if (await approveButton.count() > 0) {
			...
			const notesField = page.locator('textarea[data-fieldname="approval_notes"], textarea[placeholder*="notes"]');
			if (await notesField.count() > 0) {
				await notesField.fill('Application approved subject to standard conditions.');

				// Submit approval
				const submitButton = page.locator('button.btn-primary:has-text("Submit"), button:has-text("Approve")');
				if (await submitButton.count() > 0) {
					await submitButton.click();
					await page.waitForLoadState('networkidle');
					await page.waitForTimeout(1000);
			...
			console.log('Approve action not available - may require Manager role or Pending Decision status');
		}
	});
	...
	test('Step 11 - Verify status history', async () => {
		...
		const timeline = page.locator('.timeline, [data-fieldname="status_history"], .workflow-timeline');

		if (await timeline.count() > 0) {
			console.log('Status history/timeline found');

			// Check for timeline entries
			const timelineEntries = await page.locator('.timeline-item, .history-row').count();
	...
	test('Step 12 - Verify SLA tracking throughout workflow', async () => {
		...
		const daysElapsed = await page.locator('[data-fieldname="working_days_elapsed"] input').inputValue().catch(() => '');
	...
	test('Step 13 - Verify cost tracking', async () => {
		...
		if (await assessmentLink.count() > 0) {
			...
			// Check cost fields
			const budgetedCost = await page.locator('[data-fieldname="budgeted_cost"] input').inputValue().catch(() => '');
			const actualCost = await page.locator('[data-fieldname="actual_cost"] input').inputValue().catch(() => '');
...
test.describe('Integration Test: Multi-User Collaboration', () => {
	...
	test('01 - Planner views assigned requests', async () => {
		...
		const assignedFilter = plannerPage.locator('button:has-text("Assigned to Me"), .filter-option:has-text("My")');
	...
	test('03 - Planner processes request, Manager can see status', async () => {
		...
		if (await firstRequest.count() > 0) {
			...
			if (match) {
				...
				const plannerTitle = await plannerPage.locator('.page-title, h1').textContent();
				const managerTitle = await managerPage.locator('.page-title, h1').textContent();
