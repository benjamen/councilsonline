...
test.describe('Regression: Request Form JavaScript Fixes', () => {
	...
	test('R02 - No "Cannot read properties of undefined" errors', async () => {
		...
		if (await firstRequest.count() > 0) {
			...
			const undefinedErrors = [...consoleErrors, ...pageErrors].filter(err =>
	...
	test('R05 - Workflow progression timeline renders', async () => {
		...
		if (await firstRequest.count() > 0) {
			...
			if (await timeline.count() > 0) {
				...
				console.log('Timeline not found - may be in dashboard or different location');
			}
		}
	});
});
...
test.describe('Regression: Form Validation', () => {
	...
	test('R06 - Required fields enforced on Request form', async () => {
		...
		if (await saveButton.count() > 0) {
			...
			const validationMsg = page.locator('.msgprint, .alert, [class*="validation"]');
			...
			if (await validationMsg.count() > 0) {
				...
				console.log('Validation message not shown - may redirect or show inline errors');
			}
		}
	});
	...
	test('R07 - Email validation enforced', async () => {
		...
		const emailField = page.locator('input[data-fieldname="applicant_email"], input[type="email"]');
	...
	test('R08 - Date field validation (future dates)', async () => {
		...
		// Look for date field
		const dateField = page.locator('input[data-fieldname="target_completion_date"], input[type="date"]').first();
...
test.describe('Regression: Permission Boundaries', () => {
	...
	test('R09 - Planner cannot see Admin menu items', async () => {
		...
		const userMenu = plannerPage.locator('a[href*="/app/user"], .sidebar-item:has-text("User")');
	...
	test('R10 - Admin can see all menu items', async () => {
		...
		const userMenu = adminPage.locator('a[href*="/app/user"], .sidebar-item:has-text("User")');
	...
	test('R11 - Planner cannot approve requests (UI check)', async () => {
		...
		if (await firstRequest.count() > 0) {
			...
			if (await approveButton.count() > 0) {
				console.log('[Planner] Can see Approve button - permissions may allow this');
			} else {
	...
	test('R12 - Admin can approve requests (UI check)', async () => {
		...
		if (await firstRequest.count() > 0) {
			...
			if (await approveButton.count() > 0) {
				...
				console.log('[Admin] Approve button not shown - may require Pending Decision status');
			}
		}
	});
});
...
test.describe('Regression: Data Integrity', () => {
	...
	test('R13 - Request list stats match actual counts', async () => {
		...
		// Get reported count
		const countIndicator = page.locator('.list-count, .result-count, [class*="count"]');
	...
	test('R15 - Status history maintains chronological order', async () => {
		...
		if (await firstRequest.count() > 0) {
			...
			const historyItems = await page.locator('.timeline-item, .history-row').all();
...
test.describe('Regression: UI State Consistency', () => {
	...
	test('R16 - Applying filter updates URL and can be bookmarked', async () => {
		...
		// Apply search filter
		const searchBox = page.locator('input[type="search"], .search-input, .list-search');
	...
	test('R18 - Modal dialogs close properly without leaving overlay', async () => {
		...
		if (await firstRequest.count() > 0) {
			...
			const addButton = page.locator('button:has-text("Add Task"), button:has-text("Add")').first();
			...
			if (await addButton.count() > 0) {
				...
				// Close dialog
				const closeButton = page.locator('.modal .close, button:has-text("Close"), .modal-header .close');
