...
test.describe("RFI Workflow - Single RFI Cycle", () => {
	...
	test("Step 2: Verify RFI communication is logged", async () => {
		...
		const communicationsTab = page.locator(
			'a:has-text("Communications"), a:has-text("Timeline"), .form-timeline',
		)
		...
		if ((await communicationsTab.count()) > 0) {
			...
			const timelineContent = await page
				.locator(
					'.form-timeline, .comment-timeline, [data-fieldname="communications"]',
				)
				.textContent()

			const hasRFI =
				timelineContent.includes("RFI") ||
			...
			console.log(
				"Communications section not found - may be in different location",
			)
		}
	})
	...
	test("Step 3: Applicant responds to RFI (simulated)", async () => {
		...
		const attachSection = page.locator(
			'.attachments-section, [data-fieldname="attachments"], .file-upload',
		)
	...
	test("Verify SLA clock pauses during RFI period", async () => {
		...
		if ((await rfiRequest.count()) > 0) {
			...
			if ((await slaIndicator.count()) > 0) {
				...
				const hasPauseIndicator =
					slaText.includes("paused") ||
...
test.describe("RFI Workflow - Edge Cases", () => {
	...
	test("Verify cannot skip RFI states (must follow: Issued → Received)", async () => {
		...
		if ((await rfiRequest.count()) > 0) {
			...
			console.log(
				`Available workflow actions from RFI Issued: ${workflowButtons.length}`,
			)
	...
	test("Verify RFI can be cancelled if no longer needed", async () => {
		...
		if ((await rfiRequest.count()) > 0) {
			...
			if ((await cancelButton.count()) > 0) {
				...
				console.log(
					"No cancel RFI action found - may require admin permissions",
				)
			}
		} else {
