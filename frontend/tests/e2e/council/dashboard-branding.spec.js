...
test.describe('Council Branding - Visual Elements', () => {
	...
	test('11 - Check for custom colors/theme', async () => {
		...
		if (await navbar.count() > 0) {
			const bgColor = await navbar.first().evaluate(el =>
...
test.describe('Council Branding - Public Pages', () => {
	...
	test('13 - Verify council branding on public landing page', async () => {
		...
		const logo = page.locator('img[alt*="Council"], img[class*="logo"], .council-logo');
...
test.describe('Council Branding - Configuration', () => {
	...
	test('16 - View council branding settings', async () => {
		...
		if (await firstCouncil.count() > 0) {
			...
			const logoField = page.locator('[data-fieldname="logo"], [data-fieldname="council_logo"]');
			const colorField = page.locator('[data-fieldname="primary_color"], [data-fieldname="brand_color"]');
			...
			if (await nameField.count() > 0) {
				const councilName = await nameField.locator('input').inputValue().catch(() => '');
	...
	test('17 - Verify council code field', async () => {
		...
		if (await firstCouncil.count() > 0) {
			...
			const codeField = page.locator('[data-fieldname="council_code"], [data-fieldname="code"]');

			if (await codeField.count() > 0) {
				const councilCode = await codeField.locator('input').inputValue().catch(() => '');
