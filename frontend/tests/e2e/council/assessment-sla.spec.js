...
test.describe("Assessment Project - Lifecycle", () => {
	...
	test("05 - View stage status", async () => {
		...
		if ((await firstAssessment.count()) > 0) {
			...
			const stageStatuses = page.locator(
				'[data-fieldname="stage_status"], [class*="stage"], .indicator',
			)
	...
	test("06 - View overall assessment status", async () => {
		...
		if ((await firstAssessment.count()) > 0) {
			...
			const statusField = page.locator(
				'[data-fieldname="overall_status"] input',
			)
	...
	test("07 - View linked tasks (if exists)", async () => {
		...
		if ((await firstAssessment.count()) > 0) {
			...
			if ((await tasksSection.count()) > 0) {
				...
				const taskCount = await tasksSection
					.locator(".count, .badge")
					.textContent()
					.catch(() => "0")
...
test.describe("SLA Clock - Tracking & Calculation", () => {
	...
	test("09 - View acknowledged date (SLA start)", async () => {
		...
		const acknowledgedDateField = page.locator(
			'[data-fieldname="acknowledged_date"] input',
		)
	...
	test("11 - Check overdue indicator (if applicable)", async () => {
		...
		const overdueIndicators = page.locator(
			'.indicator-red, [class*="overdue"], .text-danger',
		)
		...
		if ((await overdueIndicators.count()) > 0) {
			...
			console.log(
				"No overdue indicators found - no overdue requests or not displayed",
			)
		}
	})
	...
	test("12 - View working days elapsed", async () => {
		...
		if ((await daysElapsedField.count()) > 0) {
			const daysElapsed = await daysElapsedField
				.locator("input")
				.inputValue()
				.catch(() => "")
	...
	test("13 - View clock exclusion periods (if tracked)", async () => {
		...
		if ((await exclusionField.count()) > 0) {
			...
			console.log(
				"Clock exclusion tracking not found - may be calculated internally",
			)
		}
	})
})
...
test.describe("Cost Tracking - Assessment & Tasks", () => {
	...
	test("14 - View budgeted hours on assessment", async () => {
		...
		if ((await firstAssessment.count()) > 0) {
			...
			if ((await budgetedHoursField.count()) > 0) {
				const budgetedHours = await budgetedHoursField
					.locator("input")
					.inputValue()
					.catch(() => "")
	...
	test("15 - View actual hours/cost", async () => {
		...
		if ((await firstAssessment.count()) > 0) {
			...
			if ((await actualField.count()) > 0) {
				const actualValue = await actualField
					.locator("input")
					.inputValue()
					.catch(() => "")
	...
	test("17 - Verify task costs roll up to assessment", async () => {
		...
		if ((await firstAssessment.count()) > 0) {
			...
			const assessmentCostField = page.locator(
				'[data-fieldname="total_costing_amount"] input',
			)
			const assessmentCost = await assessmentCostField
				.inputValue()
				.catch(() => "0")
			...
			const tasksLink = page.locator(
				'a:has-text("Project Task"), .document-link:has-text("Task")',
			)
	...
	test("18 - View budgeted vs actual variance", async () => {
		...
		if ((await firstAssessment.count()) > 0) {
			...
			const budgetedField = page.locator(
				'[data-fieldname="expected_time"] input',
			)
			const actualField = page.locator(
				'[data-fieldname="total_actual_time"] input',
			)
			...
			if (budgeted && actual) {
				const variance = Number.parseFloat(actual) - Number.parseFloat(budgeted)
