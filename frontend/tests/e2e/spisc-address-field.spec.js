...
test.describe('SPISC Address Field Saving', () => {
    ...
    test('Verify address_line saves correctly to backend', async ({ page }) => {
        ...
        const streetInput = page.locator('input[placeholder*="Street"], input[placeholder*="street"], input[placeholder*="House"]').first()
        ...
        console.log('[Test] Step 1 saved, attempting to retrieve data from backend...')
        ...
        draftId = await page.evaluate(() => {
            return window.localStorage.getItem('current_draft_id') ||
        ...
        if (!draftId) {
            ...
            console.log('[Test] Attempting to find latest SPISC Application via API...')

            const listResponse = await page.goto(`${BACKEND_URL}/api/resource/SPISC Application?fields=["name","creation","full_name"]&limit_page_length=1&order_by=creation desc`)
        ...
        const response = await page.goto(`${BACKEND_URL}/api/resource/SPISC Application/${draftId}`)
