/**
 * Regression Test Suite for Lodgeick v1.3+ Critical Bugs (E2E)
 *
...
test.describe('BUG-001: Project Task Autoname (E2E)', () => {
	test('should create tasks from template without duplicate errors', async ({ page }) => {
		...
		const projectExists = await page.locator('.list-row').first().isVisible({ timeout: 5000 }).catch(() => false);
		...
		if (projectExists) {
			...
			const buttonExists = await createTaskButton.isVisible({ timeout: 3000 }).catch(() => false);
			...
			if (buttonExists) {
				...
				const errorDialog = page.locator('.modal-content:has-text("Duplicate entry")');
				...
				const successToast = page.locator('.alert-success, .indicator-pill.green');
				const hasSuccess = await successToast.isVisible({ timeout: 3000 }).catch(() => false);

				if (hasSuccess) {
					console.log('✓ BUG-001: Tasks created successfully without duplicate errors');
				}
			}
		}
	});
	...
	test('should display unique task names in Project Task list view', async ({ page }) => {
		...
		const taskNames = await page.locator('.list-row .level-item.bold').allTextContents();
...
test.describe('BUG-002: Council Meeting DocType (E2E)', () => {
	test('should load Council Meeting list page without 404', async ({ page }) => {
		...
		const is404 = await notFoundText.isVisible({ timeout: 2000 }).catch(() => false);
	...
	test('should create new Council Meeting successfully', async ({ page }) => {
		...
		const hasForm = await page.locator('form, .form-layout').isVisible({ timeout: 3000 }).catch(() => false);
	...
	test('should call book_council_meeting API successfully', async ({ page }) => {
		...
		const requestExists = await page.locator('.list-row').first().isVisible({ timeout: 5000 }).catch(() => false);
		...
		if (requestExists) {
			...
			const bookButton = page.locator('button:has-text("Book"), button:has-text("Meeting")');
			const hasButton = await bookButton.first().isVisible({ timeout: 3000 }).catch(() => false);
			...
			if (hasButton) {
				...
				if (response) {
					...
					console.log(`✓ BUG-002: book_council_meeting API returned status ${response.status()}`);
				}
			}
		}
	});
	...
	test('should not have Pre-Application Meeting references', async ({ page }) => {
		...
		const notFound = page.locator('text=not found, text=Page pre-application-meeting not found');
...
test.describe('BUG-003: SPISC Application Draft Save (E2E)', () => {
	test('should save draft SPISC request without validation error', async ({ page }) => {
		...
		const fullNameField = page.locator('[data-fieldname="requester"], input[name="full_name"]').first();
		const fullNameExists = await fullNameField.isVisible({ timeout: 3000 }).catch(() => false);
		...
		const saveButtonExists = await saveDraftButton.isVisible({ timeout: 3000 }).catch(() => false);

		if (saveButtonExists) {
			// Set up error listener
			const errorDialog = page.locator('.modal-content:has-text("Fetch From"), .msgprint:has-text("Fetch From")');

			await saveDraftButton.click();
			await page.waitForTimeout(3000);

			// CRITICAL ASSERTION: Should NOT see fetch_from validation error
			const hasFetchFromError = await errorDialog.isVisible({ timeout: 2000 }).catch(() => false);
			...
			if (hasError) {
				...
				console.log('✓ BUG-003: No fetch_from error (other validation may exist)');
			} else {
				console.log('✓ BUG-003: Draft saved without fetch_from validation error');
			}
		}
	});

	test('should display applicant name correctly after save', async ({ page }) => {
		test.setTimeout(90000);
		...
		const appExists = await page.locator('.list-row').first().isVisible({ timeout: 5000 }).catch(() => false);
		...
		if (appExists) {
			...
			const applicantNameField = page.locator('[data-fieldname="applicant_name"]');
			const hasApplicantName = await applicantNameField.isVisible({ timeout: 3000 }).catch(() => false);

			if (hasApplicantName) {
				const applicantNameValue = await applicantNameField.inputValue().catch(() => '');

				// CRITICAL ASSERTION: applicant_name should be populated (not empty)
				expect(applicantNameValue).not.toBe('');

				console.log(`✓ BUG-003: Applicant name populated: "${applicantNameValue}"`);
			}
		}
	});
	...
	test('should create SPISC application with age field populated', async ({ page }) => {
		...
		if (appExists) {
			...
			const ageDisplayField = page.locator('[data-fieldname="applicant_age_display"]');
			const hasAgeDisplay = await ageDisplayField.isVisible({ timeout: 3000 }).catch(() => false);
...
test.describe('Regression Suite Summary', () => {
	test('should verify all critical DocTypes exist', async ({ page }) => {
		...
		await page.goto('/app/project-task');
		await page.waitForLoadState('networkidle');
		let notFound = await page.locator('text=not found').isVisible({ timeout: 2000 }).catch(() => false);
		...
		await page.goto('/app/council-meeting');
		await page.waitForLoadState('networkidle');
		notFound = await page.locator('text=not found').isVisible({ timeout: 2000 }).catch(() => false);
		expect(notFound).toBe(false);
