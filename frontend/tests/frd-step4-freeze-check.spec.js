...
test('Test FRD Resource Consent - Step 4 Component Load (Freeze Check)', async ({ page }) => {
  const consoleLogs = []
  ...
  if (await resourceConsentCard.isVisible().catch(() => false)) {
    ...
    const toRawFixLog = consoleLogs.find(log => log.includes('[Step1ApplicantProposal] Using toRaw to prevent freeze'))

    console.log('\n=== COMPONENT LOADING STATUS ===')
    console.log('Component loaded with toRaw fix:', toRawFixLog ? '✅' : '❌')

    if (!toRawFixLog) {
      console.error('❌ FREEZE DETECTED: Component did not load or toRaw fix not applied')
