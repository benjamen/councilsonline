...
test('Test Step 3 - Check for Infinite Loop', async ({ page }) => {
  ...
  const buildingConsentCard = page.locator('text=Building Consent - Residential New Build').first()
  ...
  if (step4Logs.length > 5) {
    ...
    console.error(`Step 4 validation called ${step4Logs.length} times in 3 seconds`)
    console.error('First 5 logs:', step4Logs.slice(0, 5))
    throw new Error(`Infinite loop detected: Step 4 validation called ${step4Logs.length} times`)
  }
