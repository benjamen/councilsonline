...
test('Test Step 4 Applicant Details - All Form Options', async ({ page }) => {
  ...
  if (await resourceConsentCard.isVisible().catch(() => false)) {
    ...
    const redundantQuestion = page.locator('text=This application is for myself')
    const questionExists = await redundantQuestion.isVisible({ timeout: 500 }).catch(() => false)

    if (questionExists) {
      console.error('❌ FAIL: Redundant "Who is this application for?" section still exists!')
    ...
    // Applicant type
    const applicantTypeSelect = page.locator('select').filter({ hasText: 'Select applicant type' }).or(page.locator('select >> nth=0'))
    await applicantTypeSelect.selectOption('Individual')
    await page.waitForTimeout(500)
    console.log('✅ Applicant type selected')

    await page.screenshot({ path: 'test-results/step4-test-applicant-filled.png' })
    ...
    const ownerSection = page.locator('div.flex:has-text("I am not the property owner")')
    const notOwnerCheckbox = ownerSection.locator('input[type="checkbox"]').first()

    // Check the box
    await notOwnerCheckbox.check()
    await page.waitForTimeout(1000)
    await page.screenshot({ path: 'test-results/step4-test-owner-fields-shown.png' })

    // Verify owner fields appear
    const ownerNameInput = page.locator('input[placeholder*="Property owner"], input[placeholder*="owner"]').first()
    const ownerFieldsVisible = await ownerNameInput.isVisible({ timeout: 1000 }).catch(() => false)

    if (!ownerFieldsVisible) {
      console.error('❌ FAIL: Owner fields did not appear when checkbox checked')
      ...
      const ownerEmailInput = page.locator('input[placeholder*="owner"]').filter({ hasText: '' }).or(
        page.locator('label:has-text("Owner Email")').locator('~ input')
      ).first()
      ...
      await page.screenshot({ path: 'test-results/step4-test-owner-filled.png' })
    }

    // Uncheck to hide owner fields
    await notOwnerCheckbox.uncheck()
    await page.waitForTimeout(1000)
    const ownerFieldsHidden = !(await ownerNameInput.isVisible({ timeout: 500 }).catch(() => false))

    if (!ownerFieldsHidden) {
      console.error('❌ FAIL: Owner fields did not hide when checkbox unchecked')
    ...
    const depositCheckbox = page.locator('label:has-text("Transfer deposit from existing consent")')
    const depositExists = await depositCheckbox.isVisible({ timeout: 500 }).catch(() => false)

    if (depositExists) {
      console.error('❌ FAIL: Deposit transfer checkbox still exists in Step 4!')
      await page.screenshot({ path: 'test-results/step4-test-delivery-still-exists.png' })
      throw new Error('Delivery & Payment section was not removed from Step 4')
    } else {
      console.log('✅ PASS: Delivery & Payment section successfully removed from Step 4')
    }

    // Test 5: Verify form can be submitted (Next button works)
    console.log('Test 5: Testing form submission...')
    const nextBtn = page.locator('button:has-text("Next")')
    const nextBtnEnabled = await nextBtn.isEnabled({ timeout: 1000 }).catch(() => false)

    if (!nextBtnEnabled) {
      console.log('⚠️  Next button disabled - this is expected if required fields are missing')
    } else {
    ...
    console.log('✅ Test 2: Applicant contact information fields work correctly')
