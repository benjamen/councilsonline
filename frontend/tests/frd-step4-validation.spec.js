...
test('Test FRD Resource Consent - Step 4 Validation (No Infinite Loop)', async ({ page }) => {
  const consoleLogs = []
  const step4Logs = []
  ...
  if (await resourceConsentCard.isVisible().catch(() => false)) {
    ...
    if (step4Logs.length > 5) {
      ...
      console.error(`Step 4 validation called ${step4Logs.length} times while on Step 3`)
      await page.screenshot({ path: 'test-results/frd-step3-frozen.png' })
      throw new Error(`Infinite loop on Step 3: ${step4Logs.length} validation calls`)
    }
    ...
    try {
      ...
      await page.click('button:has-text("Next")', { force: true, timeout: 2000 })
    }
    ...
    if (step4Logs.length > 10) {
      ...
      console.error(`Step 4 validation called ${step4Logs.length} times in 2 seconds`)
      console.error('Logs:', step4Logs.slice(0, 5))
      throw new Error(`Infinite loop during form interaction: ${step4Logs.length} validation calls`)
    }
