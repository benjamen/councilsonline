...
test.describe("Council Website Flow", () => {
	test("should complete SPISC application from council link", async ({
		page,
	}) => {
		...
		await page.goto("http://localhost:8090/?council=TAYTAY-PH", {
		...
		await page.goto(
			"http://localhost:8080/new-request?council=TAYTAY-PH&locked=true",
			{ waitUntil: "networkidle" },
		)
