...
test.describe("SPISC Dynamic Steps - Council Link Flow", () => {
	test("CRITICAL: should show all 5 SPISC dynamic steps from council link", async ({
		page,
		context,
	}) => {
		...
		await expect(page.locator("h2")).toContainText("Household Information", {
			timeout: 10000,
		})
		...
		await expect(page.locator("h2")).toContainText("Supporting Documents", {
			timeout: 10000,
		})
		...
		await expect(page.locator("h2")).toContainText("Declaration", {
			timeout: 10000,
		})
	...
	test("should allow navigation back through dynamic steps", async ({
		page,
		context,
	}) => {
