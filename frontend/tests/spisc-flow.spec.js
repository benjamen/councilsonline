...
test.describe("SPISC Request Flow", () => {
	...
	test("should display single progress bar with correct step count", async ({
		page,
	}) => {
		...
		const progressText = await page
			.locator("text=/Process Info|Council Process Information/")
			.first()
			.textContent()
		...
		const percentageText = await page
			.locator("text=/\\d+% Complete/")
			.textContent()
		console.log("Percentage:", percentageText)
		const percentage = Number.parseInt(percentageText.match(/(\d+)%/)[1])
	...
	test("should save draft successfully", async ({ page }) => {
		...
		const has417Error = errors.some(
			(err) => err.includes("417") || err.includes("EXPECTATION FAILED"),
		)
	...
	test("should not show duplicate address fields", async ({ page }) => {
		...
		const municipalityInputs = await page
			.locator('input[name="municipality"]')
			.count()
	...
	test("should resume draft at saved step when editing", async ({ page }) => {
		...
		const draftIdPromise = page.waitForResponse(
			(response) =>
				response.url().includes("create_draft_request") &&
				response.status() === 200,
		)
		...
		const progressText = await page
			.locator(".text-sm.font-medium.text-gray-700")
			.first()
			.textContent()
		expect(progressText).toContain("Personal Information")

		console.log(
			"✓ Draft resumed successfully at saved step with all data restored",
		)
	})
})
