...
test.describe('Council Website Integration', () => {
  test('should load and display request types from council link', async ({ page }) => {
    ...
    await expect(page.locator('h2')).toContainText('Select Application Type', { timeout: 5000 });
    ...
    await expect(page.locator('text=Social Pension for Indigent Senior Citizens').first()).toBeVisible();
    await expect(page.locator('text=Local Senior Assistance').first()).toBeVisible();
    await expect(page.locator('text=Burial / Medical Support').first()).toBeVisible();
  ...
  test('should allow selecting and progressing with SPISC', async ({ page }) => {
  ...
  test('should handle council parameter without locked flag', async ({ page }) => {
    console.log('=== Test: Council Param Without Locked Flag ===');

    await page.goto('http://localhost:8080/frontend/request/new?council=TAYTAY-PH', {
      waitUntil: 'domcontentloaded'
    });
