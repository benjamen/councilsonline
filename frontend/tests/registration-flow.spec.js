...
test.describe('Registration Flow', () => {
  ...
  test('Applicant Registration - Individual with Property', async ({ page }) => {
    ...
    const addressInput = page.locator('input[placeholder*="Start typing address"]')
  ...
  test('Applicant Registration - Company', async ({ page }) => {
    ...
    const companyNumberInput = page.locator('input[placeholder*="Company Number"]')
  ...
  test('Property Address Lookup Integration', async ({ page }) => {
    ...
    if (await dropdown.count() > 0) {
      ...
      if (await noResults.count() > 0) {
        console.log('✓ No results message shown (property API may not be running)')
      }
    } else {
      console.log('ℹ Address lookup dropdown not shown (property API may not be running)')
    }
