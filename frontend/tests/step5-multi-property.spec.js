...
test('Test Step 5 Multi-Property Details - Add Multiple Properties', async ({ page }) => {
  // Login
  ...
  if (await resourceConsentCard.isVisible().catch(() => false)) {
    ...
    const propertyDetailsHeading = page.locator('h2:has-text("Property Details")')
    const propertyDetailsVisible = await propertyDetailsHeading.isVisible({ timeout: 2000 }).catch(() => false)
    ...
    const addButtonVisible = await addPropertyButton.isVisible({ timeout: 2000 }).catch(() => false)

    if (!addButtonVisible) {
      console.error('❌ FAIL: Add Property button not found!')
      await page.screenshot({ path: 'test-results/step5-multi-test-no-add-button.png' })
    ...
    const emptyStateVisible = await emptyState.isVisible({ timeout: 1000 }).catch(() => false)

    if (!emptyStateVisible) {
      console.log('⚠️  WARNING: Empty state not visible (might have default property)')
    } else {
      console.log('✅ PASS: Empty state visible')
    }
    await page.screenshot({ path: 'test-results/step5-multi-test-empty-state.png' })
    ...
    if (!modalVisible) {
      ...
      await page.screenshot({ path: 'test-results/step5-multi-test-modal-failed.png' })
      throw new Error('Modal did not open')
    } else {
      console.log('✅ PASS: Property modal opened')
    }
    await page.screenshot({ path: 'test-results/step5-multi-test-modal-open.png' })

    // Test 5: Verify LINZ search field exists in modal
    console.log('Test 5: Verifying LINZ search field...')
    const searchInput = page.locator('input[placeholder*="Start typing address"]')
    const searchInputVisible = await searchInput.isVisible({ timeout: 1000 }).catch(() => false)
    ...
    await page.screenshot({ path: 'test-results/step5-multi-test-search-results.png' })
    ...
    const modalClosed = !(await modalHeading.isVisible({ timeout: 500 }).catch(() => false))
    if (!modalClosed) {
    ...
    await page.screenshot({ path: 'test-results/step5-multi-test-modal-closed.png' })
    ...
    const consentInfoHeading = page.locator('h2:has-text("Consent Information"), h3:has-text("Consent Information")')
    const consentInfoExists = await consentInfoHeading.isVisible({ timeout: 1000 }).catch(() => false)
    ...
    if (consentInfoExists) {
      ...
      if (consentInfoPosition && propertyDetailsPosition) {
        if (consentInfoPosition.y < propertyDetailsPosition.y) {
          console.log('✅ PASS: Consent Information appears before Property Details (correct order)')
        } else {
          console.error('❌ FAIL: Property Details appears before Consent Information (wrong order)')
    ...
    console.log('✅ Test 8: Section ordering correct (Consent Info before Property Details)')
