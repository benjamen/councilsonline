...
test('SPISC save draft with filled form', async ({ page }) => {
	...
	const isLoggedIn = await page.locator('button:has-text("New Request")').isVisible().catch(() => false)
	...
	if (!clicked) {
		...
		await page.screenshot({ path: 'test-results/spisc-not-found.png', fullPage: true })
	...
	// Check for errors
	const errorVisible = await page.locator('text=/error|Error|failed|Failed/i').isVisible({ timeout: 2000 }).catch(() => false)

	if (errorVisible) {
		await page.screenshot({ path: 'test-results/save-draft-error.png', fullPage: true })
