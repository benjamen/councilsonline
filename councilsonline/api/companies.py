...
def send_invitation_email(invitation, company, user_exists):
	...
	subject = _("Invitation to join {0} on CouncilsOnline").format(company.company_name)
	...
	if user_exists:
		...
		message = f"""
		<h2>You've been invited to join {company.company_name} on CouncilsOnline</h2>

		<p>You have been invited to join {company.company_name} as a <strong>{invitation.role}</strong>.</p>

		<p>Since you don't have a CouncilsOnline account yet, you'll need to register first, then accept this invitation.</p>
