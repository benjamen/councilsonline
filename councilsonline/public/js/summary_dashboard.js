...
frappe.provide('councilsonline.dashboard');
 ...
 */
councilsonline.dashboard.createSummaryDashboard = function(frm, config = {}) {
	const default_config = {
		api_method: 'councilsonline.api.get_request_summary_data',
	...
	frappe.call({
		...
		callback: function(r) {
			if (r.message) {
				councilsonline.dashboard.renderDashboard(frm, dashboard_config, r.message);
			}
		}
	});
};
 ...
 */
councilsonline.dashboard.renderDashboard = function(frm, config, data) {
 ...
 */
councilsonline.dashboard.createSimpleCountDashboard = async function(frm, count_configs) {
	const counts = {};

	// Fetch all counts in parallel
	await Promise.all(
		count_configs.map(async (cfg) => {
			counts[cfg.doctype] = await councilsonline.actionBar.getLinkedDocumentCount(frm.doc.name, cfg.doctype);
		})
	);
	...
	councilsonline.dashboard.renderDashboard(frm, { metrics }, data);
};
 ...
 */
councilsonline.dashboard.updateDashboardMetric = function(frm, metric_label, new_value) {
 ...
 */
councilsonline.dashboard.createMetricCard = function(label, value, icon, color = 'white') {
	return `
		<div class="dashboard-metric" style="flex: 1; text-align: center; padding: 10px;">
			<div style="font-size: 14px; color: rgba(255,255,255,0.9); margin-bottom: 5px;">
				<svg class="icon icon-sm" style="width: 16px; height: 16px; margin-right: 5px; vertical-align: middle;">
