...
frappe.provide('councilsonline.workflow');

/**
 * Get workflow steps based on request type
 */
councilsonline.workflow.getWorkflowSteps = function(request_type, request_category) {
 ...
 */
councilsonline.workflow.getStepState = function(step_key, current_state, all_steps) {
 ...
 */
councilsonline.workflow.renderWorkflowProgression = function(workflow_state, request_type, request_category) {
    if (!workflow_state) {
        return '<p class="text-muted">No workflow state</p>';
    }

    const steps = councilsonline.workflow.getWorkflowSteps(request_type, request_category);
    const completed_count = steps.filter(step => councilsonline.workflow.getStepState(step.key, workflow_state, steps) === 'completed').length;
    const progress_percentage = (completed_count / steps.length) * 100;

    const steps_html = steps.map((step, index) => {
        const state = councilsonline.workflow.getStepState(step.key, workflow_state, steps);
 ...
 */
councilsonline.workflow.renderWorkflowIndicator = function(workflow_state, request_type, request_category) {
    const steps = councilsonline.workflow.getWorkflowSteps(request_type, request_category);
 ...
 */
councilsonline.workflow.getNextStep = function(workflow_state, request_type, request_category) {
    const steps = councilsonline.workflow.getWorkflowSteps(request_type, request_category);
    const current_index = steps.findIndex(s => s.key === workflow_state);
 ...
 */
councilsonline.workflow.getPreviousStep = function(workflow_state, request_type, request_category) {
    const steps = councilsonline.workflow.getWorkflowSteps(request_type, request_category);
    const current_index = steps.findIndex(s => s.key === workflow_state);
